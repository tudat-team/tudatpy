...
The :ref:`model_settings` module provides a list of observation model settings (range, Doppler, etc.) to use, while the :ref:`biases`, :ref:`light_time_corrections` and :ref:`links` provide interfaces for creating the various input to the observation model settings. A more detailed overview of the different setting types and their interaction is given on the `user guide <https://docs.tudat.space/en/latest/user-guide/state-estimation/observation-model-setup.html>`_ .
