...
The primary link with the rest of Tudat is that the :class:`~tudatpy.estimation.observations_setup.observations_simulation_settings.ObservationSimulationSettings` objects are used as input to :func:`~tudatpy.estimation.observations.simulate_observation_dataset` to generate simulated observations. More details and examples on the procedure to simulate observations are given on the `user guide <https://docs.tudat.space/en/latest/user-guide/state-estimation/observation-simulation/creating-observations/simulating-observations.html#simulating-observations>`_.
