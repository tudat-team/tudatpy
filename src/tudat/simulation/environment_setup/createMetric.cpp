    ...
    switch( spaceTimeMetricSettings->getMetricType( ) )
    {
        case schwarzschild_metric: {
            ...
            spaceTimeMetric = std::make_shared< relativity::HarmonicSchwarzschildMetric >(
                    std::bind( &gravitation::GravityFieldModel::getGravitationalParameter, centralBody->getGravityFieldModel( ) ),
                    ppnSet,
                    bodyName,
        ...
        case solar_system_metric: {
            ...
            for( unsigned int i = 0; i < firstOrderBodies.size( ) + secondOrderBodies.size( ); ++i )
            {
                ...
                if( body->getEphemeris( ) != nullptr )
                {
                    ...
                    bodyAccelerationFunctions.push_back( [ ephemeris, accelerationStep ]( const double t ) {
                        return ephemeris->getCartesianAcceleration( t, accelerationStep );
                    } );
                }
                else
                {
                    bodyAccelerationFunctions.push_back( []( const double ) { return Eigen::Vector3d::Zero( ); } );
                }
                ...
                if( harmonicMap.count( currentBody ) > 0 )
                {
                    std::shared_ptr< gravitation::SphericalHarmonicsGravityField > harmonicField =
                            std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravityField >( body->getGravityFieldModel( ) );
                    ...
                    {
                        throw std::runtime_error( "Error: Body " + currentBody + " has SH metric settings but no rotational ephemeris." );
                    }

                    std::function< Eigen::Matrix3d( ) > rotationDerivativeFunction =
                            std::bind( &Body::getCurrentRotationMatrixDerivativeToLocalFrame, body );
                    std::function< Eigen::Quaterniond( ) > rotationFunction = std::bind( &Body::getCurrentRotationToLocalFrame, body );

                    rotationUpdateFunctions[ i ] = [ body, currentBody ]( const double t ) {
                    ...
                    bodySphericalHarmonicGravityWrappers[ i ] = std::make_shared< SphericalHarmonicWrapper >(
                            ...
                            std::make_shared< basic_mathematics::SphericalHarmonicsCache >( harmonicMap.at( currentBody ).first + 1,
            ...
            spaceTimeMetric = std::make_shared< relativity::SolarSystemMetric >( bodyList,
        ...
        default:
            throw std::runtime_error( "Error when creating space-time metric: metric type " +
