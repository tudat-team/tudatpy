...
InterArcStateContinuityConstraintSettings::InterArcStateContinuityConstraintSettings(
    ...
    body_( std::move( body ) ),
    connectionEpochs_( std::move( connectionEpochs ) ),
    weightMatrices_( std::move( weightMatrices ) ),
    muValues_( std::move( muValues ) ),
    arcPairs_( std::move( arcPairs ) )
{
    validate( );
}
...
void InterArcStateContinuityConstraintSettings::validate( ) const
{
    ...
    if( connectionEpochs_.empty( ) )
    {
        throw std::runtime_error( "Error in InterArcStateContinuityConstraintSettings for body " + body_ +
                                  ": connectionEpochs is empty." );
    }
    if( !arcPairs_.empty( ) && arcPairs_.size( ) != connectionEpochs_.size( ) )
    {
        throw std::runtime_error( "Error in InterArcStateContinuityConstraintSettings for body " + body_ +
                                  ": arcPairs size (" + std::to_string( arcPairs_.size( ) ) +
                                  ") does not match connectionEpochs size (" + std::to_string( connectionEpochs_.size( ) ) + ")." );
    }
...
void InterArcStateContinuityConstraintSettings::validateWeightMatrix(
        const Eigen::Matrix< double, 6, 6 >& C, std::size_t entryIndex ) const
{
    ...
    {
        throw std::runtime_error( "Error in InterArcStateContinuityConstraintSettings for body " + body_ +
                                  ": weight matrix entry " + std::to_string( entryIndex ) +
                                  " is not symmetric (asymmetry norm = " + std::to_string( symmetryNorm ) + ")." );
    }
    ...
    if( smallestEigenValue < psdTolerance )
    {
        throw std::runtime_error( "Error in InterArcStateContinuityConstraintSettings for body " + body_ +
                                  ...
                                  std::to_string( smallestEigenValue ) + ")." );
    }
}
