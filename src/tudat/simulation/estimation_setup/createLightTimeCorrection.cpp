    ...
    switch( correctionSettings->getCorrectionType( ) )
    {
        ...
        case tabulated_tropospheric: {
                  ...
                  receiver.bodyName_ == troposphericCorrectionSettings->getBodyWithAtmosphere( ) ) )
            {
                ...
                {
                    throw std::runtime_error( "Error when creating tabulated tropospheric corrections for " + groundStation.getReferencePointName() +
        ...
        case saastamoinen_tropospheric: {
                  ...
                  receiver.bodyName_ == troposphericCorrectionSettings->getBodyWithAtmosphere( ) ) )
            {
                ...
                lightTimeCorrection = std::make_shared< SaastamoinenTroposphericCorrection >(
                        ...
                        bodies.getBody( groundStation.bodyName_ )->getGroundStation( groundStation.getReferencePointName() )->getPressureFunction( ),
                        bodies.getBody( groundStation.bodyName_ )
                                ->getGroundStation( groundStation.getReferencePointName() )
                                ->getTemperatureFunction( ),
        ...
        case vmf3o_tropospheric: {
            ...
            if( correctionSettings == nullptr )
            {
                throw std::runtime_error( "Error when creating VMF3 tropospheric correction: incompatible settings type." );
            }
                  ...
                  receiver.bodyName_ == vmf3Settings->getBodyWithAtmosphere( ) ) )
            {
                bool isUplinkCorrection;
                LinkEndId groundStation;
                if( transmitter.bodyName_ == vmf3Settings->getBodyWithAtmosphere( ) )
                {
                    isUplinkCorrection = true;
                    groundStation = transmitter;
                }
                ...
                std::shared_ptr< ground_stations::StationTroposphereData > troposphereData =
                        bodies.getBody( groundStation.bodyName_ )->getGroundStation( groundStation.getReferencePointName() )->getTroposphereData( );
        ...
        case tabulated_ionospheric: {
                  ...
                  receiver.bodyName_ == ionosphericCorrectionSettings->getBodyWithAtmosphere( ) ) )
            {
                ...
                {
                    throw std::runtime_error( "Error when creating tabulated ionospheric corrections for " + groundStation.getReferencePointName() +
        ...
        case ionex_vtec_ionospheric: {
            std::shared_ptr< IonexIonosphericCorrectionSettings > ionosphericCorrectionSettings =
                    std::dynamic_pointer_cast< IonexIonosphericCorrectionSettings >( correctionSettings );
            if( correctionSettings == nullptr )
            {
                throw std::runtime_error( "Error when creating Global TEC ionospheric correction: incompatible settings type." );
            }
            // check that the Earth ionosphere is defined, else throw an error

            // If one of the link ends is the body with the atmosphere then create the ionospheric correction
            if( transmitter.bodyName_ != receiver.bodyName_ &&
                ( transmitter.bodyName_ == ionosphericCorrectionSettings->getBodyWithIonosphere( ) ||
                  receiver.bodyName_ == ionosphericCorrectionSettings->getBodyWithIonosphere( ) ) )
            {
                bool isUplinkCorrection;
                LinkEndId groundStation, spacecraft;
                if( transmitter.bodyName_ == ionosphericCorrectionSettings->getBodyWithIonosphere( ) )
                {
                    isUplinkCorrection = true;
                    groundStation = transmitter;
                    spacecraft = receiver;
                ...
                {
                    throw std::runtime_error( "Error when creating Global TEC ionospheric corrections for body " + groundStation.bodyName_ +
                                              ": shape model not recognized" );
                }
    ...
    switch( troposphericMappingModelType )
    {
        ...
        case TroposphericMappingModel::niell: {
            ...
            std::function< Eigen::Vector3d( double ) > groundStationGeodeticPositionFunction = std::bind(
                    ...
                    bodies.getBody( groundStation.bodyName_ )->getGroundStation( groundStation.getReferencePointName( ) )->getNominalStationState( ) );
        ...
        const LinkEndType& receivingLinkEndType )
{
    ...
    if( bodies.getBody( linkEnds.at( transmitter ).bodyName_ )->getGroundStationMap( ).count( linkEnds.at( transmitter ).getReferencePointName() ) >
        0 )
    {
         ...
         ++retransmitterLinkEndsIt )
    {
        ...
        if( vehicleSystems == nullptr )
        {
            throw std::runtime_error( "Error when creating link frequency function: vehicle systems are not defined for " +
                                      retransmitterLinkEndsIt->second.bodyName_ + "/" + retransmitterLinkEndsIt->second.getReferencePointName() +
                                      " link end." );
        }
