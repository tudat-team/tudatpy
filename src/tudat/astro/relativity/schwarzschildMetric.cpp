...
void HarmonicSchwarzschildMetric::updateMetric( )
{
    ...
    const double scaledPotential = currentCentralBodyGravitationalParameter_ * physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT / distance;
    ...
    if( includeSecondPostNewtonianOrder_ )
    {
        currentSecondOrderCovariantMetricContributions_.block( 1, 1, 3, 3 ) +=
                ...
                4.0 *
                        calculateExternalMatrixPotential(
                                currentCentralBodyGravitationalParameter_, distance, currentEvaluationState_.segment( 0, 3 ) ) *
                        physical_constants::INVERSE_QUARTIC_SPEED_OF_LIGHT;
    }

    currentCovariantMetricContribution_ = currentFirstOrderCovariantMetricContributions_ + currentSecondOrderCovariantMetricContributions_;
}
...
void HarmonicSchwarzschildMetric::updateChristoffelSymbols( )
{
    ...
    currentChristoffelSymbols_[ 0 ].block( 1, 1, 3, 3 ) = scalarPotentialTimeDerivative * gamma * Eigen::Matrix3d::Identity( );
    ...
    for( int i = 0; i < 3; i++ )
    {
        ...
        for( int j = 0; j < 3; j++ )
        {
            ...
            {
                spatialBlock( j, k ) =
                        gamma * ( ( j == k ? position( i ) : 0.0 ) - ( i == j ? position( k ) : 0.0 ) - ( i == k ? position( j ) : 0.0 ) );
            }
        }
        ...
        if( includeSecondPostNewtonianOrder_ )
        {
            for( int j = 0; j < 3; j++ )
            {
                for( int k = 0; k < 3; k++ )
                {
                    spatialBlock( j, k ) += 2.0 * scaledPotential * ( epsilon - gamma * gamma ) * ( j == k ? position( i ) : 0.0 );

                    spatialBlock( j, k ) += -scaledPotential * ( epsilon - 2.0 * gamma * gamma ) *
                            ( ( i == j ? position( k ) : 0.0 ) + ( i == k ? position( j ) : 0.0 ) );
                }
            }
        }
    ...
    if( includeSecondPostNewtonianOrder_ )
    {
        Eigen::Matrix3d secondOrderCorrection = -position * position.transpose( ) * inverseDistanceSquared * epsilon * scaledPotential;
        for( int i = 0; i < 3; i++ )
        {
