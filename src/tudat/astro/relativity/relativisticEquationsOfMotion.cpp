...
Eigen::Vector4d evaluateFourAcceleration( const std::shared_ptr< relativity::Metric > spaceTimeMetric,
                                          const Eigen::Matrix< double, 8, 1 >& currentFourState )
{
    return evaluateFourAcceleration( spaceTimeMetric->getCurrentChristoffelSymbols( ), currentFourState.segment( 4, 4 ) );
}
...
{
    return evaluateRelativisticEquationsOfMotionInCoordinateTime( spaceTimeMetric->getCurrentChristoffelSymbols( ),
...
double evaluateProperTimeEquation( const Eigen::Matrix4d& covariantMetricPerturbation,
                                   const Eigen::Vector3d& coordinateVelocity,
    ...
    for( int i = 1; i <= squareRootOrderExpansion; i++ )
    {
        switch( i )
        {
            ...
            case 3:
                ...
                break;
            default:
                throw std::runtime_error( "Error when calculating proper time equation, cannot expand square root at order " +
                                          std::to_string( i ) );
        }
    }
...
double evaluateProperTimeEquation( const std::shared_ptr< relativity::Metric > spaceTimeMetric,
                                   ...
                                   const int squareRootOrderExpansion )
{
    return evaluateProperTimeEquation(
            spaceTimeMetric->getCurrentCovariantMetricPeturbation( ), currentState.segment( 3, 3 ), squareRootOrderExpansion );
}
