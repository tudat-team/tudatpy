...
void SolarSystemMetric::updateMetric( )
{
    ...
    currentCovariantMetricContribution_( 0, 0 ) = 2.0 *
            ( currentTotalExternalScalarPotential_ * physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT -
    ...
    Eigen::Vector3d spaceTimeCoupling = -( ppnParameterSet_->getParameterGamma( ) + 1.0 ) * 2.0 * currentVectorPotential_ *
    ...
    double secondOrderPpnParameter =
            ppnParameterSet_->getParameterGamma( ) * ppnParameterSet_->getParameterGamma( ) + ppnParameterSet_->getParameterBeta( ) - 1.0;

    double spaceSpaceDiagonalContribution = 2.0 * ppnParameterSet_->getParameterGamma( ) * currentTotalExternalScalarPotential_ *
                    physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT +
            2.0 * secondOrderPpnParameter * currentSecondOrderPotentialSquaredTerms_ * physical_constants::INVERSE_QUARTIC_SPEED_OF_LIGHT;
    ...
    currentCovariantMetricContribution_.block( 1, 1, 3, 3 ) += 2.0 * physical_constants::INVERSE_QUARTIC_SPEED_OF_LIGHT *
...
void SolarSystemMetric::updateCurrentState( const bool updateEvaluationPointIndependentVariables )
{
    ...
    {
        currentSphericalHarmonicPotentials_[ index ] = wrapper->getCurrentPotential( currentEvaluationState_.segment( 0, 3 ) );
    }
}
...
void SolarSystemMetric::updatePotentialPartials( )
{
    ...
    for( size_t i = 0; i < bodyList_.size( ); ++i )
    {
        ...
        if( bodySphericalHarmonicGravityWrappers_.count( static_cast< int >( i ) ) > 0 )
        {
            ...
            if( r > 0.0 )
            {
                centralAcceleration = -currentBodyGravitationalParameters_[ i ] / ( r * r * r ) * currentBodyRelativePositions_[ i ];
            }
            const Eigen::Vector3d nonCentralAcceleration = currentScalarPotentialGradients_[ i ] - centralAcceleration;
        ...
        double scalarTimeDeriv = currentScalarPotentialGradients_[ i ].dot( -currentBodyStates_[ i ].segment( 3, 3 ) ) + inherentTimeDeriv;
        ...
        currentVectorPotentialTimePartial_ += currentExternalScalarPotentials_[ i ] * currentBodyAccelerations_[ i ] +
                currentBodyStates_[ i ].segment( 3, 3 ) * scalarTimeDeriv;

        currentInherentVectorPotentialTimePartial_ += currentBodyStates_[ i ].segment( 3, 3 ) * inherentTimeDeriv;
...
void SolarSystemMetric::updateChristoffelSymbols( )
{
    ...
    currentChristoffelSymbols_[ 0 ]( 0, 0 ) = -physical_constants::INVERSE_CUBIC_SPEED_OF_LIGHT * currentScalarPotentialTimePartial_;
    ...
    currentChristoffelSymbols_[ 0 ].block( 1, 1, 3, 3 ) = physical_constants::INVERSE_CUBIC_SPEED_OF_LIGHT *
    ...
    Eigen::Vector3d timeTimeTerms = -physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT * currentScalarPotentialPositionPartial_ +
            physical_constants::INVERSE_QUARTIC_SPEED_OF_LIGHT *
    ...
    Eigen::Matrix3d spaceTimeTerms = physical_constants::INVERSE_CUBIC_SPEED_OF_LIGHT *
            ( -( ppnParameterSet_->getParameterGamma( ) + 1.0 ) *
                      ( currentVectorPotentialPositionPartial_ - currentVectorPotentialPositionPartial_.transpose( ) ) +
              ppnParameterSet_->getParameterGamma( ) * Eigen::Matrix3d::Identity( ) * currentScalarPotentialTimePartial_ );
    ...
    for( int i = 1; i < 4; ++i )
    {
        ...
        currentChristoffelSymbols_[ i ].block( 1, 1, 3, 3 ) -= physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT *
                ppnParameterSet_->getParameterGamma( ) * ( Eigen::Matrix3d::Identity( ) * currentScalarPotentialPositionPartial_( i - 1 ) );

        currentChristoffelSymbols_[ i ].block( i, 1, 1, 3 ) += ppnParameterSet_->getParameterGamma( ) *
                physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT * currentScalarPotentialPositionPartial_.transpose( );

        currentChristoffelSymbols_[ i ].block( 1, i, 3, 1 ) += ppnParameterSet_->getParameterGamma( ) *
                physical_constants::INVERSE_SQUARE_SPEED_OF_LIGHT * currentScalarPotentialPositionPartial_;
    }
}
