...
double calculateFirstOrderExternalScalarPotential(
        ...
        const std::map< int, double >& sphericalHarmonicPotentials )
{
    std::vector< double > externalScalarPotentials = calculateFirstOrderExternalScalarPotentials(
                gravitationalParameters, distancesFromBody, sphericalHarmonicPotentials );
                                                              ...
                                                              const std::vector< Eigen::Vector3d >& accelerations )
{
    ...
    for( unsigned int i = 0; i < gravitationalParameters.size( ); i++ )
    {
        ...
        singleBodyCorrectionTerm += -2.0 * velocities[ i ] * velocities[ i ] + 0.5 *
                ( velocityPositionProduct * velocityPositionProduct / ( distancesFromBody[ i ] * distancesFromBody[ i ] ) )
                + 4.0 * stateVectors[ i ].segment( 3, 3 ).dot( centralBodyVelocity );// Final term included in Fienga et al 2009, but no Soffel et al. // +
        ...
        for( unsigned int j = 0; j < gravitationalParameters.size( ); j++ )
        {
            if( i != j )
            {
                singleBodyCorrectionTerm += gravitationalParameters[ j ] / ( ( stateVectors[ j ].segment( 0, 3 ) - stateVectors[ i ].segment( 0, 3 ) ).norm( ) );
            }
        }
        ...
        const std::map< int, Eigen::Vector3d >& currentAngularMomenta )
{
    ...
    for( std::map< int, Eigen::Vector3d >::const_iterator momentaIterator = currentAngularMomenta.begin( );
         momentaIterator != currentAngularMomenta.end( ); momentaIterator++ )
    {
        currentDistance = distancesFromBody.at( momentaIterator->first );
        vectorPotentials.at( momentaIterator->first ) -= physical_constants::GRAVITATIONAL_CONSTANT * ( momentaIterator->second ).cross(
                    Eigen::Vector3d( stateVectors.at( momentaIterator->first ).segment( 0, 3 ) ) ) /
...
Eigen::Vector3d calculateExternalVectorPotential(
        ...
        const std::map< int, Eigen::Vector3d >& currentAngularMomenta )
{
    std::vector< Eigen::Vector3d > externalVectorPotentials = calculateExternalVectorPotentials(
                gravitationalParameters, distancesFromBody, stateVectors, currentAngularMomenta );
        ...
        const std::vector< double >& distancesFromBody )
{
         ...
         momentaIterator != currentAngularMomenta.end( ); momentaIterator++ )
    {
        ...
        externalVectorPotential -= physical_constants::GRAVITATIONAL_CONSTANT * ( momentaIterator->second ).cross(
                    Eigen::Vector3d( stateVectors.at( momentaIterator->first ).segment( 0, 3 ) ) ) /
                ( 2.0 * currentDistance * currentDistance * currentDistance );
    }
    return externalVectorPotential;
...
{
    Eigen::Matrix3d matrixPotential = ( gravitationalParameter * gravitationalParameter ) /
                ( distanceFromBody * distanceFromBody * 4.0 ) * (
        ...
        const std::vector< Eigen::Vector3d >& relativePositions )
{
    ...
    for( unsigned int i = 0; i < gravitationalParameters.size( ); i++ )
    {
        matrixPotential += calculateExternalMatrixPotential(
                    gravitationalParameters.at( i ), distancesFromBody.at( i ), relativePositions.at( i ) );
    }
    return matrixPotential;
}
