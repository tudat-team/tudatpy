...
ComaWindModel::ComaWindModel( const simulation_setup::ComaPolyDataset& xPolyDataset,
    ...
    WindModel( associatedFrame, includeCorotation, useRadius ), dataType_( 0 ),  // POLYNOMIAL_COEFFICIENTS
    maximumDegree_( maximumDegree ), maximumOrder_( maximumOrder ), isLog2Data_( isLog2Data ), xIsZeroComponent_( false ),
    yIsZeroComponent_( false ), zIsZeroComponent_( false ),
    ...
    zPolyDataset_( std::make_shared< simulation_setup::ComaPolyDataset >( zPolyDataset ) ), xStokesDataset_( nullptr ),
    yStokesDataset_( nullptr ), zStokesDataset_( nullptr ), comaModel_( comaModel ), sunStateFunction_( std::move( sunStateFunction ) ),
    cometStateFunction_( std::move( cometStateFunction ) ), cometRotationFunction_( std::move( cometRotationFunction ) ),
    sphericalHarmonicsCalculator_( nullptr ), cachedSolarLongitude_( 0.0 ), cachedTime_( -std::numeric_limits< double >::infinity( ) ),
    lastXFileIndex_( 0 ), lastYFileIndex_( 0 ), lastZFileIndex_( 0 ), cachedRadius_( 0.0 ), cachedEffectiveRadius_( 0.0 ),
    cachedInterpolationSolarLongitude_( 0.0 ), coefficientMatricesSized_( false ), cachedLatitude_( 0.0 ), cachedLongitude_( 0.0 ),
    cachedFinalWindVector_( Eigen::Vector3d::Zero( ) ), interpolationPoint2D_( 2 ), interpolationPoint1D_( 1 )
{
    if( !comaModel_ )
    {
    ...
    if( isLog2Data_ )
    {
        auto polyAllZero = []( const simulation_setup::ComaPolyDataset& ds ) -> bool {
            for( std::size_t f = 0; f < ds.getNumFiles( ); ++f )
                if( ds.getPolyCoefficients( f ).any( ) ) return false;
            return true;
        };
...
ComaWindModel::ComaWindModel( const simulation_setup::ComaStokesDataset& xStokesDataset,
                              ...
                              const bool useRadius,
                              const bool isLog2Data ):
    WindModel( associatedFrame, includeCorotation, useRadius ), dataType_( 1 ),  // STOKES_COEFFICIENTS
    maximumDegree_( maximumDegree ), maximumOrder_( maximumOrder ), isLog2Data_( isLog2Data ), xIsZeroComponent_( false ),
    yIsZeroComponent_( false ), zIsZeroComponent_( false ), xPolyDataset_( nullptr ), yPolyDataset_( nullptr ), zPolyDataset_( nullptr ),
    ...
    zStokesDataset_( std::make_shared< simulation_setup::ComaStokesDataset >( zStokesDataset ) ), comaModel_( comaModel ),
    sunStateFunction_( std::move( sunStateFunction ) ), cometStateFunction_( std::move( cometStateFunction ) ),
    cometRotationFunction_( std::move( cometRotationFunction ) ), sphericalHarmonicsCalculator_( nullptr ), cachedSolarLongitude_( 0.0 ),
    cachedTime_( -std::numeric_limits< double >::infinity( ) ), lastXFileIndex_( 0 ), lastYFileIndex_( 0 ), lastZFileIndex_( 0 ),
    cachedRadius_( 0.0 ), cachedEffectiveRadius_( 0.0 ), cachedInterpolationSolarLongitude_( 0.0 ), coefficientMatricesSized_( false ),
    cachedLatitude_( 0.0 ), cachedLongitude_( 0.0 ), cachedFinalWindVector_( Eigen::Vector3d::Zero( ) ), interpolationPoint2D_( 2 ),
            ...
            for( std::size_t f = 0; f < ds.nFiles( ); ++f )
                for( std::size_t r = 0; r < ds.nRadii( ); ++r )
                    for( std::size_t l = 0; l < ds.nLongitudes( ); ++l )
                        if( ds.block( f, r, l ).any( ) ) return false;
            return true;
        };
                                                                         ...
                                                                         const double currentTime )
{
    ...
    if( cacheFlags_.windVectorValid && radiusDiff * radiusDiff < toleranceSq && lonDiff * lonDiff < toleranceSq &&
        latDiff * latDiff < toleranceSq && timeDiff * timeDiff < toleranceSq )
    {
 ...
 */
Eigen::Vector3d ComaWindModel::computeWindVectorFromStokesCoefficients( double radius, double longitude, double latitude, double time )
{
    ...
    const bool sameRadius = cacheFlags_.interpolationValid && radiusDiff * radiusDiff < radiusToleranceSq;
    const bool sameSolarLongitude = cacheFlags_.interpolationValid && solarLongitudeDiff * solarLongitudeDiff < solarLongitudeToleranceSq;
    ...
    if( !sameRadius || !sameSolarLongitude )
    {
        ...
        {
            if( cosineCoeffs->rows( ) == effectiveMaxDegree + 1 && cosineCoeffs->cols( ) == effectiveMaxOrder + 1 )
            {
        ...
        {
            if( sineCoeffs->rows( ) == effectiveMaxDegree + 1 && sineCoeffs->cols( ) == effectiveMaxOrder + 1 )
            {
                sineCoeffs->setZero( );
            }
            else
            ...
            if( fileIndex < static_cast< int >( xStokesInterpolators_.size( ) ) && !xStokesInterpolators_[ fileIndex ].empty( ) )
            {
                ...
                for( int degree = 0; degree <= effectiveMaxDegree; ++degree )
                {
                    for( int order = 0; order <= std::min( degree, effectiveMaxOrder ); ++order )
                    {
                        ...
                        {
                            createFallback2DInterpolator( xStokesDataset_,
                        ...
                        {
                            createFallback2DInterpolator( yStokesDataset_,
                        ...
                        {
                            createFallback2DInterpolator( zStokesDataset_,
    ...
    const double windX = shCalc->calculateSurfaceSphericalHarmonics(
            cachedXSineCoefficients_, cachedXCosineCoefficients_, latitude, longitude, effectiveMaxDegree, effectiveMaxOrder );

    const double windY = shCalc->calculateSurfaceSphericalHarmonics(
            cachedYSineCoefficients_, cachedYCosineCoefficients_, latitude, longitude, effectiveMaxDegree, effectiveMaxOrder );

    const double windZ = shCalc->calculateSurfaceSphericalHarmonics(
            cachedZSineCoefficients_, cachedZCosineCoefficients_, latitude, longitude, effectiveMaxDegree, effectiveMaxOrder );

    return Eigen::Vector3d( ( isLog2Data_ && !xIsZeroComponent_ ) ? std::exp2( windX ) : windX,
 ...
 * \throws std::runtime_error If datasets are null or time is out of range
 */
Eigen::Vector3d ComaWindModel::computeWindVectorFromPolyCoefficients( double radius, double longitude, double latitude, double time )
{
    ...
    if( !coefficientMatricesSized_ || cachedXCosineCoefficients_.rows( ) != effectiveMaxDegree + 1 ||
    ...
    simulation_setup::StokesCoefficientsEvaluator::evaluate2D( effectiveRadiusX,
                                                               solarLongitude,
                                                               xPolyCoefficients,
                                                               xShIndices,
                                                               xFileMeta.powersInvRadius,
                                                               xFileMeta.referenceRadius,
                                                               cachedXCosineCoefficients_,
                                                               cachedXSineCoefficients_,
                                                               maximumDegree_,
                                                               maximumOrder_,
                                                               isLog2Data_ );
    ...
    simulation_setup::StokesCoefficientsEvaluator::evaluate2D( effectiveRadiusY,
                                                               solarLongitude,
                                                               yPolyCoefficients,
                                                               yShIndices,
                                                               yFileMeta.powersInvRadius,
                                                               yFileMeta.referenceRadius,
                                                               cachedYCosineCoefficients_,
                                                               cachedYSineCoefficients_,
                                                               maximumDegree_,
                                                               maximumOrder_,
                                                               isLog2Data_ );
    ...
    simulation_setup::StokesCoefficientsEvaluator::evaluate2D( effectiveRadiusZ,
                                                               solarLongitude,
                                                               zPolyCoefficients,
                                                               zShIndices,
                                                               zFileMeta.powersInvRadius,
                                                               zFileMeta.referenceRadius,
                                                               cachedZCosineCoefficients_,
                                                               cachedZSineCoefficients_,
                                                               maximumDegree_,
                                                               maximumOrder_,
                                                               isLog2Data_ );
    ...
    const double windX =
            shCalc->calculateSurfaceSphericalHarmonics( cachedXSineCoefficients_,
                                                        cachedXCosineCoefficients_,
                                                        latitude,
                                                        longitude,
    ...
    const double windY =
            shCalc->calculateSurfaceSphericalHarmonics( cachedYSineCoefficients_,
                                                        cachedYCosineCoefficients_,
                                                        latitude,
                                                        longitude,
    ...
    const double windZ =
            shCalc->calculateSurfaceSphericalHarmonics( cachedZSineCoefficients_,
                                                        cachedZCosineCoefficients_,
                                                        latitude,
                                                        longitude,
...
int ComaWindModel::findTimeIntervalIndex( double time, std::shared_ptr< simulation_setup::ComaPolyDataset > dataset )
{
    ...
    int& cachedFileIndex = ( dataset == xPolyDataset_ ) ? lastXFileIndex_
            : ( dataset == yPolyDataset_ )              ? lastYFileIndex_
                                                        : lastZFileIndex_;
...
int ComaWindModel::findTimeIntervalIndex( double time, std::shared_ptr< simulation_setup::ComaStokesDataset > dataset )
{
    ...
    int& cachedFileIndex = ( dataset == xStokesDataset_ ) ? lastXFileIndex_
            : ( dataset == yStokesDataset_ )              ? lastYFileIndex_
                                                          : lastZFileIndex_;
 ...
 */
double ComaWindModel::computeWindComponentFromPolyCoefficients( std::shared_ptr< simulation_setup::ComaPolyDataset > dataset,
                                                                double radius,
                                                                double longitude,
                                                                double latitude,
                                                                double time )
{
    ...
    Eigen::MatrixXd& cosineCoefficients = ( dataset == xPolyDataset_ ) ? cachedXCosineCoefficients_
            : ( dataset == yPolyDataset_ )                             ? cachedYCosineCoefficients_
                                                                       : cachedZCosineCoefficients_;
    Eigen::MatrixXd& sineCoefficients = ( dataset == xPolyDataset_ ) ? cachedXSineCoefficients_
            : ( dataset == yPolyDataset_ )                           ? cachedYSineCoefficients_
                                                                     : cachedZSineCoefficients_;
    ...
    // Check sizing flag first to avoid repeated size comparisons
    if( !coefficientMatricesSized_ || cosineCoefficients.rows( ) != effectiveMaxDegree + 1 ||
    ...
    simulation_setup::StokesCoefficientsEvaluator::evaluate2D( effectiveRadius,
                                                               solarLongitude,
                                                               polyCoefficients,
                                                               shIndices,
        ...
        double time )
{
    ...
    if( !sameRadius || !sameSolarLongitude )
    {
        ...
        if( cosineCoefficients.rows( ) == effectiveMaxDegree + 1 && cosineCoefficients.cols( ) == effectiveMaxOrder + 1 )
        {
            cosineCoefficients.setZero( );
            sineCoefficients.setZero( );
        }
                ...
                for( int degree = 0; degree <= effectiveMaxDegree; ++degree )
                {
                    for( int order = 0; order <= std::min( degree, effectiveMaxOrder ); ++order )
                    {
                        ...
                        else
                        {
                            ...
                            createFallback2DInterpolator( dataset,
                                                          fallbackInterpolators,
                                                          fileIndex,
                                                          degree,
                                                          order,
    ...
    return getActiveSurfaceSphericalHarmonicsCalculator( )->calculateSurfaceSphericalHarmonics(
            sineCoefficients, cosineCoefficients, latitude, longitude, effectiveMaxDegree, effectiveMaxOrder );
}
        ...
        for( int degree = 0; degree <= effectiveMaxDegree; ++degree )
        {
            for( int order = 0; order <= std::min( degree, effectiveMaxOrder ); ++order )
            {
                ...
                // X-component reduced interpolators
                {
                    ...
                    std::pair< int, int > degreeOrderPair = { degree, order };
                    xReducedStokesInterpolators_[ fileIndex ][ degreeOrderPair ] = { std::move( reducedCosineInterpolator ),
                                                                                     std::move( reducedSineInterpolator ) };
                }
                ...
                // Y-component reduced interpolators
                {
                    ...
                    std::pair< int, int > degreeOrderPair = { degree, order };
                    yReducedStokesInterpolators_[ fileIndex ][ degreeOrderPair ] = { std::move( reducedCosineInterpolator ),
                                                                                     std::move( reducedSineInterpolator ) };
                }
                ...
                // Z-component reduced interpolators
                {
                    ...
                    std::pair< int, int > degreeOrderPair = { degree, order };
                    zReducedStokesInterpolators_[ fileIndex ][ degreeOrderPair ] = { std::move( reducedCosineInterpolator ),
                                                                                     std::move( reducedSineInterpolator ) };
                }
            }
        }
