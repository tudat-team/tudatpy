...
{
    // Compute density
    double waveModelTerm = uncertaintyFactor + dustStormFactor + 0.1 * std::sin( 1.0 * longitude ) +     // first longitudinal wave
    ...
    return exponentialAtmosphereModel( altitude, longitude, latitude, time, densityAtReferenceAltitude, referenceAltitude, scaleHeight ) *
            waveModelTerm;
}
...
CustomConstantTemperatureAtmosphere::CustomConstantTemperatureAtmosphere(
    ...
    constantTemperature_( constantTemperature ), specificGasConstant_( specificGasConstant ), ratioOfSpecificHeats_( ratioOfSpecificHeats )
{
        ...
        case exponential_atmosphere_model: {
            ...
            if( modelSpecificParameters.size( ) != 3 )
            {
                throw std::runtime_error(
                        "Error while creating custom constant temperature atmosphere model. The "
                        "number of input model-dependent parameters is incorrect. Number of "
                        "parameters given " +
                        std::to_string( modelSpecificParameters.size( ) ) + ". Number required: 3." );
            }
        ...
        case three_wave_atmosphere_model: {
            ...
            if( modelSpecificParameters.size( ) != 5 )
            {
                throw std::runtime_error(
                        ...
                        "number of input model-dependent parameters is incorrect. Number of "
                        "parameters given " +
                        std::to_string( modelSpecificParameters.size( ) ) + ". Number required: 5." );
            }
        ...
        case three_term_atmosphere_model: {
            ...
            if( modelSpecificParameters.size( ) != 6 )
            {
                throw std::runtime_error(
                        ...
                        "number of input model-dependent parameters is incorrect. Number of "
                        "parameters given " +
                        std::to_string( modelSpecificParameters.size( ) ) + ". Number required: 6." );
            }
