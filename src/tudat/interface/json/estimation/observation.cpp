...
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< ObservationModelSettings >& observationSettings )
{
    ...
    const ObservableType observableType = getValue< ObservableType >( jsonObject, K::observableType );
    ...
    std::vector< std::shared_ptr< LightTimeCorrectionSettings > > lightTimeCorrectionsList =
            getValue< std::vector< std::shared_ptr< LightTimeCorrectionSettings > > >(
                    jsonObject, K::lightTimeCorrectionSettingsList, std::vector< std::shared_ptr< LightTimeCorrectionSettings > >( ) );
    std::shared_ptr< ObservationBiasSettings > biasSettings =
            getValue< std::shared_ptr< ObservationBiasSettings > >( jsonObject, K::biasSettings, nullptr );
    ...
    switch( observableType )
    {
        ...
        case n_way_range: {
            ...
            {
                observationSettings =
                        std::make_shared< ObservationModelSettings >( observableType, lightTimeCorrectionsList, biasSettings );
            }
            else
            {
...
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< ObservationBiasSettings >& biasSettings )
{
    ...
    const ObservationBiasTypes biasType = getValue< ObservationBiasTypes >( jsonObject, K::biasType );

    switch( biasType )
    {
        case constant_absolute_bias: {
            Eigen::VectorXd constantBias = getValue< Eigen::VectorXd >( jsonObject, K::constantBias );
        ...
        case arc_wise_constant_absolute_bias: {
            std::vector< double > arcStartTimes = getValue< std::vector< double > >( jsonObject, K::arcStartTimes );
            std::vector< Eigen::VectorXd > observationBiases = getValue< std::vector< Eigen::VectorXd > >( jsonObject, K::arcWiseBiasList );
            LinkEndType linkEndForTime = getValue< LinkEndType >( jsonObject, K::referenceLinkEnd );
            biasSettings =
                    std::make_shared< ArcWiseConstantObservationBiasSettings >( arcStartTimes, observationBiases, linkEndForTime, true );
            return;
        }
        case arc_wise_constant_relative_bias: {
            std::vector< double > arcStartTimes = getValue< std::vector< double > >( jsonObject, K::arcStartTimes );
            ...
            LinkEndType linkEndForTime = getValue< LinkEndType >( jsonObject, K::referenceLinkEnd );
            biasSettings =
                    std::make_shared< ArcWiseConstantObservationBiasSettings >( arcStartTimes, observationBiases, linkEndForTime, false );
            return;
        }
        case multiple_observation_biases: {
...
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< DopplerProperTimeRateSettings >& properTimeRateSettings )
{
    ...
    const DopplerProperTimeRateType rateType = getValue< DopplerProperTimeRateType >( jsonObject, K::properTimeRateType );
...
void from_json( const nlohmann::json& jsonObject, std::shared_ptr< LightTimeCorrectionSettings >& lightTimeCorrectionSettings )
{
    ...
    try
    {
        const LightTimeCorrectionType correctionType = getValue< LightTimeCorrectionType >( jsonObject, K::lightTimeCorrectionType );
              ...
              const std::shared_ptr< ObservationSimulationTimeSettings< double > >& observationSimulationTimeSettings )
{
    ...
    if( std::dynamic_pointer_cast< TabulatedObservationSimulationTimeSettings< double > >( observationSimulationTimeSettings ) != nullptr )
    {
        std::vector< double > simulationTimes =
                std::dynamic_pointer_cast< TabulatedObservationSimulationTimeSettings< double > >( observationSimulationTimeSettings )
                        ->simulationTimes_;
...
tudat::observation_models::LinkEnds lexical_cast( const std::string& s )
{
    ...
    {
        std::vector< std::string > typeAndIdentifier = tudat::json_interface::split( individualLinkEnds.at( i ), ':' );

        std::vector< std::string > splitLinkEnd =
                tudat::json_interface::split( typeAndIdentifier.at( 1 ).substr( 1, typeAndIdentifier.at( 1 ).size( ) - 2 ), ',', false );
