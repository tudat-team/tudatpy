...
void to_json( nlohmann::json& jsonObject, const std::shared_ptr< EphemerisSettings >& ephemerisSettings )
{
    ...
    switch( ephemerisType )
    {
        case approximate_planet_positions: {
            ...
            jsonObject[ K::useCircularCoplanarApproximation ] = approximatePlanetPositionSettings->getUseCircularCoplanarApproximation( );
            return;
        }
        case direct_spice_ephemeris: {
            ...
            jsonObject[ K::correctForStellarAberration ] = directSpiceEphemerisSettings->getCorrectForStellarAberration( );
            jsonObject[ K::correctForLightTimeAberration ] = directSpiceEphemerisSettings->getCorrectForLightTimeAberration( );
            jsonObject[ K::convergeLighTimeAberration ] = directSpiceEphemerisSettings->getConvergeLighTimeAberration( );
            return;
        }
        ...
        case kepler_ephemeris: {
            ...
            jsonObject[ K::initialStateInKeplerianElements ] = keplerEphemerisSettings->getInitialStateInKeplerianElements( );
            jsonObject[ K::epochOfInitialState ] = keplerEphemerisSettings->getEpochOfInitialState( );
            jsonObject[ K::centralBodyGravitationalParameter ] = keplerEphemerisSettings->getCentralBodyGravitationalParameter( );
            jsonObject[ K::rootFinderAbsoluteTolerance ] = keplerEphemerisSettings->getRootFinderAbsoluteTolerance( );
            jsonObject[ K::rootFinderMaximumNumberOfIterations ] = keplerEphemerisSettings->getRootFinderMaximumNumberOfIterations( );
            return;
        }
        default:
    ...
    switch( ephemerisType )
    {
        case approximate_planet_positions: {
            ephemerisSettings = std::make_shared< ApproximatePlanetPositionSettings >(
                    getValue< ApproximatePlanetPositionsBase::BodiesWithEphemerisData >( jsonObject, K::bodyIdentifier ),
        ...
        case direct_spice_ephemeris: {
            ...
            ephemerisSettings = std::make_shared< DirectSpiceEphemerisSettings >(
                    ...
                    getValue( jsonObject, K::correctForStellarAberration, defaults.getCorrectForStellarAberration( ) ),
                    getValue( jsonObject, K::correctForLightTimeAberration, defaults.getCorrectForLightTimeAberration( ) ),
                    getValue( jsonObject, K::convergeLighTimeAberration, defaults.getConvergeLighTimeAberration( ) ) );
            break;
        }
        case tabulated_ephemeris: {
            TabulatedEphemerisSettings defaults( ( std::map< double, Eigen::Vector6d >( ) ) );
            TabulatedEphemerisSettings tabulatedEphemerisSettings(
                    getValue< std::map< double, Eigen::Vector6d > >( jsonObject, K::bodyStateHistory, defaults.getBodyStateHistory( ) ) );
        ...
        case interpolated_spice: {
            ...
            interpolatedSpiceEphemerisSettings.setUseLongDoubleStates(
                    getValue( jsonObject, K::useLongDoubleStates, defaults.getUseLongDoubleStates( ) ) );
            ephemerisSettings = std::make_shared< InterpolatedSpiceEphemerisSettings >( interpolatedSpiceEphemerisSettings );
            break;
        }
        case constant_ephemeris: {
            ConstantEphemerisSettings defaults( ( Eigen::Vector6d( ) ) );
            ephemerisSettings =
                    std::make_shared< ConstantEphemerisSettings >( getValue< Eigen::Vector6d >( jsonObject, K::constantState ) );
            break;
        }
        case kepler_ephemeris: {
            ...
            ephemerisSettings = std::make_shared< KeplerEphemerisSettings >(
                    ...
                    defaults.getFrameOrigin( ),
                    defaults.getFrameOrientation( ),
                    getValue( jsonObject, K::rootFinderAbsoluteTolerance, defaults.getRootFinderAbsoluteTolerance( ) ),
                    getValue( jsonObject, K::rootFinderMaximumNumberOfIterations, defaults.getRootFinderMaximumNumberOfIterations( ) ) );
            break;
        }
        default:
            handleUnimplementedEnumValue( ephemerisType, ephemerisTypes, unsupportedEphemerisTypes );
