...
{
std::map< double, std::vector< Eigen::Vector6d > > readInpopStateEphemeris( const std::string& positionFileName,
                                                                            ...
                                                                            const double referenceJulianDay )
{
    ...
    double distanceMultiplier =
            ( positionHeader.at( 4 ) == "km" ) ? 1000.0 : throw std::runtime_error( "Unknown position unit: " + positionHeader.at( 4 ) );

    std::vector< std::string > velocityHeader = parseHeader( velocityStream );
    if( std::stoi( velocityHeader.at( 6 ) ) != order || std::stoi( velocityHeader.at( 7 ) ) != span ||
    ...
    while( true )
    {
        ...
        for( int i = 0; i < 3; ++i )
        {
            if( !std::getline( positionStream, line ) ) return chebyshevCoefficients;
        ...
        for( int i = 0; i < 3; ++i )
        {
            if( !std::getline( velocityStream, line ) ) return chebyshevCoefficients;
            std::replace( line.begin( ), line.end( ), 'D', 'E' );
            std::replace( line.begin( ), line.end( ), 'd', 'e' );
            std::vector< std::string > tokens = splitLine( line );
...
std::shared_ptr< ephemerides::Ephemeris > createInpopEphemerisFromFiles( const std::string& positionFileName,
                                                                         const std::string& velocityFileName,
                                                                         const double referenceJulianDay,
