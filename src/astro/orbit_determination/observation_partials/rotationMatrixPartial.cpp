        ...
        const double timeSinceEpoch )
{
    Eigen::Matrix3d commonTerm =
            Eigen::AngleAxisd( -1.0 * ( -initialOrientationAngles.z( ) - rotationRate * timeSinceEpoch ), Eigen::Vector3d::UnitZ( ) )
                    .toRotationMatrix( );
    ...
    Eigen::Matrix3d rightAscensionPartial =
            -reference_frames::getDerivativeOfZAxisRotationWrtAngle( -( rightAscension + mathematical_constants::PI / 2.0 ) );

    // Compute partial of rotation term containing declination.
    Eigen::Matrix3d declinationPartial =
            reference_frames::getDerivativeOfXAxisRotationWrtAngle( -( mathematical_constants::PI / 2.0 - declination ) );

    // Compute partials.
    std::vector< Eigen::Matrix3d > rotationMatrixPartials;
    rotationMatrixPartials.push_back(
            rightAscensionPartial *
            Eigen::Matrix3d( Eigen::AngleAxisd( -( declination - mathematical_constants::PI / 2.0 ), Eigen::Vector3d::UnitX( ) ) ) *
            commonTerm );
    rotationMatrixPartials.push_back(
            Eigen::AngleAxisd( ( rightAscension + mathematical_constants::PI / 2.0 ), Eigen::Vector3d::UnitZ( ) ).toRotationMatrix( ) *
        ...
        const double timeSinceEpoch )
{
    std::vector< Eigen::Matrix3d > partialsOfRotationMatrix =
            calculatePartialOfRotationMatrixFromLocalFrameWrtPoleOrientation( initialOrientationAngles, rotationRate, timeSinceEpoch );
    partialsOfRotationMatrix[ 0 ] =
            -rotationRate * partialsOfRotationMatrix.at( 0 ) * reference_frames::Z_AXIS_ROTATION_MATRIX_DERIVATIVE_PREMULTIPLIER;
    partialsOfRotationMatrix[ 1 ] =
            -rotationRate * partialsOfRotationMatrix.at( 1 ) * reference_frames::Z_AXIS_ROTATION_MATRIX_DERIVATIVE_PREMULTIPLIER;
        ...
        const double ephemerisTime )
{
    ...
    for( std::map< double, std::pair< double, double > >::iterator correctionIterator = rotationrateCorrections.begin( );
         ...
         correctionIterator++ )
    {
        rotationMatrixPartials.push_back( std::cos( correctionIterator->first * currentMeanAnomaly ) *
        ...
        rotationMatrixPartials.push_back( std::sin( correctionIterator->first * currentMeanAnomaly ) *
                                          rotationFromMeanOrbitToIcrf.toRotationMatrix( ) *
                                          ( Eigen::AngleAxisd( currentAngleCorrections.x( ), Eigen::Vector3d::UnitZ( ) ) *
                                            Eigen::AngleAxisd( currentAngleCorrections.y( ), Eigen::Vector3d::UnitX( ) ) )
                                                  .toRotationMatrix( ) *
        ...
        const double ephemerisTime )
{
    ...
    double currentMeanAnomaly = planetaryOrientationCalculator->getBodyMeanAnomalyAtEpoch( ) + meanMotion * ephemerisTime;
         ...
         correctionIterator++ )
    {
        partialsOfRotationMatrix.push_back(
                ...
                ( Eigen::AngleAxisd( currentAngleCorrections.x( ), Eigen::Vector3d::UnitZ( ) ) *
                  Eigen::AngleAxisd( currentAngleCorrections.y( ), Eigen::Vector3d::UnitX( ) ) )
                        .toRotationMatrix( ) *
                ( Eigen::Matrix3d( ) << correctionIterator->first * meanMotion *
                                          std::sin( correctionIterator->first * currentMeanAnomaly ) * std::sin( currentPhiAngle ) -
                                  std::cos( correctionIterator->first * currentMeanAnomaly ) * std::cos( currentPhiAngle ) *
                                          meanPhiAngleDerivative,
                  correctionIterator->first * meanMotion * std::sin( correctionIterator->first * currentMeanAnomaly ) *
                                  std::cos( currentPhiAngle ) +
                          std::cos( correctionIterator->first * currentMeanAnomaly ) * std::sin( currentPhiAngle ) * meanPhiAngleDerivative,
                  0.0,
                  -correctionIterator->first * meanMotion * std::sin( correctionIterator->first * currentMeanAnomaly ) *
                                  std::cos( currentPhiAngle ) -
                          std::cos( correctionIterator->first * currentMeanAnomaly ) * std::sin( currentPhiAngle ) * meanPhiAngleDerivative,
                  correctionIterator->first * meanMotion * std::sin( correctionIterator->first * currentMeanAnomaly ) *
                                  std::sin( currentPhiAngle ) -
                          std::cos( correctionIterator->first * currentMeanAnomaly ) * std::cos( currentPhiAngle ) * meanPhiAngleDerivative,
                  0.0,
                  0.0,
                  0.0,
                  0.0 )
        ...
        partialsOfRotationMatrix.push_back(
                ...
                ( Eigen::AngleAxisd( currentAngleCorrections.x( ), Eigen::Vector3d::UnitZ( ) ) *
                  Eigen::AngleAxisd( currentAngleCorrections.y( ), Eigen::Vector3d::UnitX( ) ) )
                        .toRotationMatrix( ) *
                ( Eigen::Matrix3d( ) << -correctionIterator->first * meanMotion *
                                          std::cos( correctionIterator->first * currentMeanAnomaly ) * std::sin( currentPhiAngle ) -
                                  std::cos( currentPhiAngle ) * std::sin( correctionIterator->first * currentMeanAnomaly ) *
                                          meanPhiAngleDerivative,
                  -correctionIterator->first * meanMotion * std::cos( correctionIterator->first * currentMeanAnomaly ) *
                                  std::cos( currentPhiAngle ) +
                          std::sin( currentPhiAngle ) * std::sin( correctionIterator->first * currentMeanAnomaly ) * meanPhiAngleDerivative,
                  0.0,
                  correctionIterator->first * meanMotion * std::cos( correctionIterator->first * currentMeanAnomaly ) *
                                  std::cos( currentPhiAngle ) -
                          std::sin( currentPhiAngle ) * std::sin( correctionIterator->first * currentMeanAnomaly ) * meanPhiAngleDerivative,
                  -correctionIterator->first * meanMotion * std::cos( correctionIterator->first * currentMeanAnomaly ) *
                                  std::sin( currentPhiAngle ) -
                          std::cos( currentPhiAngle ) * std::sin( correctionIterator->first * currentMeanAnomaly ) * meanPhiAngleDerivative,
                  0.0,
                  0.0,
                  0.0,
                  0.0 )
    ...
    const double ephemerisTime )
{
    ...
    for ( unsigned int librationIndex = 0; librationIndex < librationFrequencies.size( ); librationIndex++ )
    {
        ...
        double currentLibrationTerm = std::sin( signature.first * ephemerisTime + signature.second );
        Eigen::AngleAxisd( -currentLibrationTerm, Eigen::Vector3d::UnitZ( )).toRotationMatrix( );
        ...
        const double ephemerisTime )
{
    ...
    std::map< double, std::pair< double, double > > xPolarMotionCoefficients =
            planetaryOrientationCalculator->getXpolarMotionCoefficients( );
    std::map< double, std::pair< double, double > > yPolarMotionCoefficients =
            planetaryOrientationCalculator->getYpolarMotionCoefficients( );
    ...
    for( std::map< double, std::pair< double, double > >::iterator xPolarMotionCoefficientIterator = xPolarMotionCoefficients.begin( );
         xPolarMotionCoefficientIterator != xPolarMotionCoefficients.end( );
         xPolarMotionCoefficientIterator++ )
    {
        rotationMatrixPartials.push_back( -std::cos( xPolarMotionCoefficientIterator->first * currentMeanAnomaly ) *
                                          rotationFromMeanOrbitToIcrf.toRotationMatrix( ) *
                                          rotationFromBodyFixedToIntermediateInertialFrame.toRotationMatrix( ) *
                                          ( Eigen::Matrix3d( ) << -std::sin( -polarMotion.x( ) ),
                                            0.0,
                                            std::cos( -polarMotion.x( ) ),
                                            0.0,
                                            0.0,
                                            0.0,
                                            -std::cos( -polarMotion.x( ) ),
                                            0.0,
                                            -std::sin( -polarMotion.x( ) ) )
                                                  .finished( ) *
                                          Eigen::AngleAxisd( -polarMotion.y( ), Eigen::Vector3d::UnitX( ) ) );

        rotationMatrixPartials.push_back( -std::sin( xPolarMotionCoefficientIterator->first * currentMeanAnomaly ) *
                                          rotationFromMeanOrbitToIcrf.toRotationMatrix( ) *
                                          rotationFromBodyFixedToIntermediateInertialFrame.toRotationMatrix( ) *
                                          ( Eigen::Matrix3d( ) << -std::sin( -polarMotion.x( ) ),
                                            0.0,
        ...
        rotationMatrixPartials.push_back( -std::cos( xPolarMotionCoefficientIterator->first * currentMeanAnomaly ) *
                                          ...
                                          ( Eigen::Matrix3d( ) << 0.0,
                                            ...
                                            0.0,
                                            0.0,
                                            -std::sin( -polarMotion.y( ) ),
                                            -std::cos( -polarMotion.y( ) ),
                                            0.0,
                                            std::cos( -polarMotion.y( ) ),
                                            -std::sin( -polarMotion.y( ) ) )
                                                  .finished( ) );
        ...
        const double ephemerisTime )
{
    ...
    for( std::map< double, std::pair< double, double > >::iterator xPolarMotionCoefficientIterator = xPolarMotionCoefficients.begin( );
         ...
         xPolarMotionCoefficientIterator++ )
    {
        partialsOfRotationMatrix.push_back( -meanPhiAngleDerivative *
        ...
        const double ephemerisTime )
{
    ...
    for( std::map< double, std::pair< double, double > >::iterator correctionIterator = meanMotionDirectNutationCorrections.begin( );
         correctionIterator != meanMotionDirectNutationCorrections.end( );
         correctionIterator++ )
    {
        double a_m = correctionIterator->first * meanMotion;

        double Psi_m = correctionIterator->second.second +
                coreFactor * a_m / ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                        ( a_m * correctionIterator->second.second +
                          freeCoreNutationRate * correctionIterator->second.first / std::sin( angleIAtEpoch ) );

        partialsOfRotationMatrix += std::sin( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) * a_m /
                ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                ( a_m * correctionIterator->second.second +
                  freeCoreNutationRate * correctionIterator->second.first / std::sin( angleIAtEpoch ) ) *
                rotationFromMeanOrbitToIcrf.toRotationMatrix( ) *
                ( Eigen::Matrix3d( ) << -std::sin( currentAngleCorrections.x( ) ),
                  -std::cos( currentAngleCorrections.x( ) ),
                  0.0,
                  std::cos( currentAngleCorrections.x( ) ),
                  -std::sin( currentAngleCorrections.x( ) ),
                  0.0,
                  0.0,
                  0.0,
                  0.0 )
        ...
        partialsOfRotationMatrix += std::cos( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) * a_m /
                ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                ( a_m * correctionIterator->second.first +
                  freeCoreNutationRate * correctionIterator->second.second * std::sin( angleIAtEpoch ) ) *
                rotationFromMeanOrbitToIcrf.toRotationMatrix( ) *
        ...
        partialsOfRotationMatrix +=
                ( Psi_m * std::sin( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) *
                          std::sin( currentAngleCorrections.y( ) ) *
                          std::cos( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) * a_m /
                          ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                          ( a_m * correctionIterator->second.first +
                            freeCoreNutationRate * correctionIterator->second.second * std::sin( angleIAtEpoch ) ) -
                  std::sin( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) * a_m /
                          ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                          ( a_m * correctionIterator->second.second +
                            freeCoreNutationRate * correctionIterator->second.first / std::sin( angleIAtEpoch ) ) *
                          std::cos( currentAngleCorrections.y( ) ) ) *
                ...
                ( Eigen::AngleAxisd( currentAngleCorrections.x( ), Eigen::Vector3d::UnitZ( ) ) *
                  Eigen::AngleAxisd( currentAngleCorrections.y( ), Eigen::Vector3d::UnitX( ) ) )
                        .toRotationMatrix( ) *
                ( Eigen::Matrix3d( ) << -std::sin( currentAngleCorrections.z( ) ),
                  -std::cos( currentAngleCorrections.z( ) ),
                  0.0,
                  std::cos( currentAngleCorrections.z( ) ),
                  -std::sin( currentAngleCorrections.z( ) ),
                  0.0,
                  0.0,
                  0.0,
                  0.0 )
    ...
    for( unsigned int i = 0; i < meanMotionTimeDependentPhaseNutationCorrections.size( ); i++ )
    {
        ...
        for( std::map< double, std::pair< double, double > >::iterator correctionIterator =
             ...
             correctionIterator != meanMotionTimeDependentPhaseNutationCorrections[ i ].end( );
             correctionIterator++ )
        {
            double a_m = correctionIterator->first * meanMotion;
            ...
            partialsOfRotationMatrix += std::sin( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) +
            ...
            partialsOfRotationMatrix += std::cos( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) +
                                                  currentPhaseAngleCorrection ) *
                    a_m / ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                    ( a_m * correctionIterator->second.first +
                      freeCoreNutationRate * correctionIterator->second.second * std::sin( angleIAtEpoch ) ) *
        ...
        const double ephemerisTime )
{
         ...
         correctionIterator++ )
    {
        ...
        partialsOfRotationMatrix += currentMeanPhiAngleDerivative *
                std::sin( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) * a_m /
                ( a_m * a_m - freeCoreNutationRate * freeCoreNutationRate ) *
                ( a_m * correctionIterator->second.second +
                  freeCoreNutationRate * correctionIterator->second.first / std::sin( angleIAtEpoch ) ) *
        ...
        partialsOfRotationMatrix += currentMeanPhiAngleDerivative *
                ...
                ( Eigen::AngleAxisd( currentAngleCorrections.x( ), Eigen::Vector3d::UnitZ( ) ) *
                  Eigen::AngleAxisd( currentAngleCorrections.y( ), Eigen::Vector3d::UnitX( ) ) )
                        .toRotationMatrix( ) *
                ( Eigen::Matrix3d( ) << -std::cos( currentAngleCorrections.z( ) ),
                  std::sin( currentAngleCorrections.z( ) ),
                  0.0,
                  -std::sin( currentAngleCorrections.z( ) ),
                  -std::cos( currentAngleCorrections.z( ) ),
        ...
        const double ephemerisTime )
{
         ...
         correctionIterator++ )
    {
        ...
        partialsOfRotationMatrix += std::sin( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) *
                ( coreFactor * a_m *
                  ( correctionIterator->second.first * ( a_m * a_m + freeCoreNutationRate * freeCoreNutationRate ) /
                            std::sin( angleIAtEpoch ) +
                    2 * freeCoreNutationRate * a_m * correctionIterator->second.second ) /
        ...
        partialsOfRotationMatrix += std::cos( correctionIterator->first * ( meanMotion * ephemerisTime + bodyMeanAnomalyAtEpoch ) ) *
                ( coreFactor * a_m *
                  ( 2 * correctionIterator->second.first * freeCoreNutationRate * a_m +
                    std::sin( angleIAtEpoch ) * ( a_m * a_m + freeCoreNutationRate * freeCoreNutationRate ) *
                            correctionIterator->second.second ) /
                  ( ( freeCoreNutationRate * freeCoreNutationRate - a_m * a_m ) *
                    ( freeCoreNutationRate * freeCoreNutationRate - a_m * a_m ) ) ) *
                rotationFromMeanOrbitToIcrf.toRotationMatrix( ) *
                ( Eigen::AngleAxisd( currentAngleCorrections.x( ), Eigen::Vector3d::UnitZ( ) ) ).toRotationMatrix( ) *
        ...
        const Eigen::Vector3d vectorInLocalFrame )
{
    ...
    Eigen::Matrix< double, 3, Eigen::Dynamic > rotatedVectorPartial =
            Eigen::Matrix< double, 3, Eigen::Dynamic >::Zero( 3, rotationMatrixPartials.size( ) );
        ...
        const Eigen::Vector3d vectorInLocalFrame )
{
    ...
    {
        throw std::runtime_error(
                "Error when caling RotationMatrixPartial::calculatePartialOfInertialVelocityWrtParameter, rotation model is nullptr" );
    }
    ...
    Eigen::Matrix3d currentRotationToTargetFrameDerivative = rotationModel_->getDerivativeOfRotationToTargetFrame( time );
    ...
    for( unsigned int i = 0; i < rotationMatrixPartials.size( ); i++ )
    {
        rotatedVectorPartial.block( 0, i, 3, 1 ) =
                -( rotationMatrixPartials.at( i ) * currentRotationToTargetFrameDerivative * currentRotationToBaseFrame +
                   ...
                   currentRotationToBaseFrame * currentRotationToTargetFrameDerivative * rotationMatrixPartials.at( i ) ) *
                vectorInLocalFrame;
    }
    return rotatedVectorPartial;
}
                                                                                        ...
                                                                                        const double scaledLibrationAmplitude )
{
    ...
    testPartial.block( 0, 0, 3, 3 ) = tudat::linear_algebra::getCrossProductMatrix( -currentState.segment( 3, 3 ) );
    testPartial.block( 0, 3, 3, 3 ) = tudat::linear_algebra::getCrossProductMatrix( currentState.segment( 0, 3 ) );
    Eigen::Vector3d crossProduct = ( currentState.segment< 3 >( 0 ).cross( currentState.segment< 3 >( 3 ) ) );
...
SynchronousRotationMatrixPartialWrtTranslationalState::calculatePartialOfRotationMatrixToBaseFrameWrParameter( const double time )
{
    // Get current state/rotation
    Eigen::Matrix3d currentRotationMatrix = synchronousRotationaModel_->getFullyLockedRotationToBaseFrame( time );
    ;
    Eigen::Vector6d currentState = synchronousRotationaModel_->getCurrentRelativeState( time );
    ...
    Eigen::Matrix3d rVectorDerivativeWrtPosition = Eigen::Matrix3d::Identity( ) / positionNorm -
            positionVector * positionVector.transpose( ) / ( positionNorm * positionNorm * positionNorm );

    // Compute partials for unnormalized w vector
    Eigen::Matrix3d unnormalizedWVectorDerivativeWrtPosition = -linear_algebra::getCrossProductMatrix( velocityVector );
    Eigen::Matrix3d unnormalizedWVectorDerivativeWrtVelocity = linear_algebra::getCrossProductMatrix( positionVector );
    Eigen::Matrix3d wPartialScalingTerm = ( Eigen::Matrix3d::Identity( ) / unnormalizedWVectorNorm -
    ...
    Eigen::Matrix3d wVectorDerivativeWrtPosition = wPartialScalingTerm * unnormalizedWVectorDerivativeWrtPosition;
    Eigen::Matrix3d wVectorDerivativeWrtVelocity = wPartialScalingTerm * unnormalizedWVectorDerivativeWrtVelocity;

    // Compute s vector partials
    Eigen::Matrix3d sVectorDerivativeWrtPosition = linear_algebra::getCrossProductMatrix( wVector ) * rVectorDerivativeWrtPosition -
            linear_algebra::getCrossProductMatrix( rVector ) * wVectorDerivativeWrtPosition;
    Eigen::Matrix3d sVectorDerivativeWrtVelocity = -linear_algebra::getCrossProductMatrix( rVector ) * wVectorDerivativeWrtVelocity;
    ...
    for( int i = 0; i < 3; i++ )
    {
        rotationMatrixPartials[ i ].block( 0, 0, 3, 1 ) = -rVectorDerivativeWrtPosition.block( 0, i, 3, 1 );
        rotationMatrixPartials[ i ].block( 0, 1, 3, 1 ) = -sVectorDerivativeWrtPosition.block( 0, i, 3, 1 );
        rotationMatrixPartials[ i ].block( 0, 2, 3, 1 ) = wVectorDerivativeWrtPosition.block( 0, i, 3, 1 );
        rotationMatrixPartials[ i ] = rotationMatrixPartials[ i ] * correctionRotation;

        rotationMatrixPartials[ i + 3 ].block( 0, 0, 3, 1 ).setZero( );
        rotationMatrixPartials[ i + 3 ].block( 0, 1, 3, 1 ) = -sVectorDerivativeWrtVelocity.block( 0, i, 3, 1 );
        rotationMatrixPartials[ i + 3 ].block( 0, 2, 3, 1 ) = wVectorDerivativeWrtVelocity.block( 0, i, 3, 1 );
    ...
    if( directLongitudeLibrationCalculator_ != nullptr )
    {
        ...
        Eigen::Matrix3d librationRotationDerivativeWrtAngle = reference_frames::getDerivativeOfZAxisRotationWrtAngle( correctionRotation );

        Eigen::Matrix< double, 1, 6 > librationDerivatives = calculatePartialOfDirectLibrationAngleWrtCartesianStates(
...
std::vector< Eigen::Matrix3d > NumericalRotationMatrixPartialWrtTranslationalState::calculatePartialOfRotationMatrixToBaseFrameWrParameter(
        const double time )
{
    ...
    for( int i = 0; i < 6; i++ )
    {
        ...
        statePartials.push_back( -( upperturbedRotationMatrix - downperturbedRotationMatrix ) / ( 2.0 * statePerturbations_( i ) ) );
    }
