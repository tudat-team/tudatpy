...
double SolarCoronaCorrection::computeMinimumDistanceOfLineOfSight( Eigen::Vector3d transmitterPositionWrtSun,
                                                                   Eigen::Vector3d receiverPositionWrtSun )
{
    ...
    // Moyer (2000), eq. 10-59
    Eigen::Vector3d minimumDistancePoint = transmitterPositionWrtSun - transmitterPositionWrtSun.dot( lineOfSight ) * lineOfSight;
...
double SolarCoronaCorrection::computeElectronDensityIntegralNumerically( const Eigen::Vector3d& transmitterPositionWrtSun,
        ...
        const std::shared_ptr< observation_models::ObservationAncilliarySimulationSettings > ancillarySettings )
{
    ...
    double sunReceiverTransmitterAngle =
            linear_algebra::computeAngleBetweenVectors( -receiverPositionWrtSun, transmitterPositionWrtSun - receiverPositionWrtSun );
    double receiverSunTransmitterAngle = linear_algebra::computeAngleBetweenVectors( receiverPositionWrtSun, transmitterPositionWrtSun );
    ...
    if( exponentsAreIntegers_ && std::abs( receiverSunTransmitterAngle ) > 0.1 * mathematical_constants::PI / 180.0 )
    {
        ...
        {
            electronDensityIntegral += coefficients_.at( i ) *
                    computeSingleTermIntegralAnalytically( receiverPositionWrtSun,
                                                           sunReceiverTransmitterAngle,
                                                           receiverSunTransmitterAngle,
...
double InversePowerSeriesSolarCoronaCorrection::computeElectronDensity( const Eigen::Vector3d& positionWrtSun, const double time )
{
...
// Verma et al., eq. A.4
double InversePowerSeriesSolarCoronaCorrection::computeSingleTermIntegralAnalytically( const Eigen::Vector3d& receiverPositionWrtSun,
...
double InversePowerSeriesSolarCoronaCorrection::computeCosinePowerIntegral( const double lowerBound,
                                                                            ...
                                                                            const unsigned int positiveExponent )
{
    ...
    if( cosinePowersIntegralsCache_.count( positiveExponent ) == 0 )
    {
        ...
        else
        {
            ...
            double term2 = static_cast< double >( positiveExponent - 1 ) / positiveExponent *
                    computeCosinePowerIntegral( lowerBound, upperBound, positiveExponent - 2 );
