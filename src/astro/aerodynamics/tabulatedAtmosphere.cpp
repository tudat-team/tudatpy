...
void checkVariableUniqueness( std::vector< VariableType > variables )
{
    ...
    // Check uniqueness
    unsigned int numberOfUniqueElements = std::distance( variables.begin( ), std::unique( variables.begin( ), variables.end( ) ) );
...
void TabulatedAtmosphere::createAtmosphereInterpolators( )
{
    ...
    if( independentVariables_.size( ) != 1 )
    {
        if( atmosphereTableFile_.size( ) != numberOfDependentVariables )
        {
            throw std::runtime_error(
                    "Error when creating tabulated atmosphere from file, "
                    "number of specified dependent variables differs from file." );
        }

        // Retrieve number of independent variables from file.
        numberOfIndependentVariables_ = input_output::getNumberOfIndependentVariablesInCoefficientFile( atmosphereTableFile_.at( 0 ) );
    ...
    if( !( dependentVariablesDependency_.at( 0 ) || dependentVariablesDependency_.at( 1 ) || dependentVariablesDependency_.at( 2 ) ) )
    {
        throw std::runtime_error(
                "Error, tabulated atmosphere must be initialized with at least "
    ...
    {
        boundaryHandling_ = std::vector< BoundaryInterpolationType >( numberOfIndependentVariables_, use_boundary_value );
    }
    else
    {
        ...
        {
            throw std::runtime_error(
                    "Error, in tabulated atmosphere. Number of boundary handling methods provided does "
    ...
    if( defaultExtrapolationValue_.empty( ) )
    {
        defaultExtrapolationValue_ = std::vector< std::vector< std::pair< double, double > > >(
                ...
                std::vector< std::pair< double, double > >( numberOfIndependentVariables_,
                                                            std::make_pair( IdentityElement::getAdditionIdentity< double >( ),
    ...
    else
    {
        ...
        {
            throw std::runtime_error(
                    "Error, in tabulated atmosphere. Number of default extrapolation values provided "
        ...
        case 1: {
            ...
            if( numberOfRowsInFile < 1 || numberOfColumnsInFile < 1 )
            {
                std::string errorMessage =
                        "Error, in tabulated atmosphere. The atmosphere table file " + atmosphereTableFile_.at( 0 ) + " is empty";
            ...
            for( unsigned int i = 0; i < numberOfRowsInFile; i++ )
            {
                ...
                for( unsigned int j = 0; j < dependentVariablesDependency_.size( ); j++ )
                {
                    ...
                    {
                        dependentVariablesData.at( dependentVariableIndices_.at( j ) )
                                .push_back( tabulatedAtmosphereData( i, dependentVariableIndices_.at( j ) + 1 ) );
                    }
                }
            }
            ...
            interpolatorForDensity_ = std::make_shared< CubicSplineInterpolatorDouble >(
                    independentVariablesData_.at( 0 ),
                    dependentVariablesData.at( dependentVariableIndices_.at( 0 ) ),
                    huntingAlgorithm,
                    boundaryHandling_.at( 0 ),
                    defaultExtrapolationValue_.at( dependentVariableIndices_.at( 0 ) ).at( 0 ) );
            interpolatorForPressure_ = std::make_shared< CubicSplineInterpolatorDouble >(
                    independentVariablesData_.at( 0 ),
                    dependentVariablesData.at( dependentVariableIndices_.at( 1 ) ),
                    huntingAlgorithm,
                    boundaryHandling_.at( 0 ),
                    defaultExtrapolationValue_.at( dependentVariableIndices_.at( 1 ) ).at( 0 ) );
            interpolatorForTemperature_ = std::make_shared< CubicSplineInterpolatorDouble >(
                    independentVariablesData_.at( 0 ),
                    dependentVariablesData.at( dependentVariableIndices_.at( 2 ) ),
                    huntingAlgorithm,
                    boundaryHandling_.at( 0 ),
                    defaultExtrapolationValue_.at( dependentVariableIndices_.at( 2 ) ).at( 0 ) );
            ...
            if( dependentVariablesDependency_.at( 3 ) )
            {
                interpolatorForGasConstant_ = std::make_shared< CubicSplineInterpolatorDouble >(
                        independentVariablesData_.at( 0 ),
                        dependentVariablesData.at( dependentVariableIndices_.at( 3 ) ),
                        huntingAlgorithm,
                        boundaryHandling_.at( 0 ),
                        defaultExtrapolationValue_.at( dependentVariableIndices_.at( 3 ) ).at( 0 ) );
            }
            if( dependentVariablesDependency_.at( 4 ) )
            {
                interpolatorForSpecificHeatRatio_ = std::make_shared< CubicSplineInterpolatorDouble >(
                        independentVariablesData_.at( 0 ),
                        dependentVariablesData.at( dependentVariableIndices_.at( 4 ) ),
                        huntingAlgorithm,
                        boundaryHandling_.at( 0 ),
                        defaultExtrapolationValue_.at( dependentVariableIndices_.at( 4 ) ).at( 0 ) );
            }
            if( dependentVariablesDependency_.at( 5 ) )
            {
                interpolatorForMolarMass_ = std::make_shared< CubicSplineInterpolatorDouble >(
                        independentVariablesData_.at( 0 ),
                        dependentVariablesData.at( dependentVariableIndices_.at( 5 ) ),
                        huntingAlgorithm,
                        boundaryHandling_.at( 0 ),
                        defaultExtrapolationValue_.at( dependentVariableIndices_.at( 5 ) ).at( 0 ) );
            }
            break;
        }
        case 2: {
...
void TabulatedAtmosphere::createMultiDimensionalAtmosphereInterpolators( )
{
    ...
    // Create interpolators for density, pressure and temperature
    interpolatorForDensity_ = std::make_shared< MultiLinearInterpolator< double, double, NumberOfIndependentVariables > >(
            independentVariablesData_,
            tabulatedAtmosphereData.first.at( dependentVariableIndices_.at( 0 ) ),
            huntingAlgorithm,
            boundaryHandling_,
            defaultExtrapolationValue_.at( dependentVariableIndices_.at( 0 ) ) );
    interpolatorForPressure_ = std::make_shared< MultiLinearInterpolator< double, double, NumberOfIndependentVariables > >(
            independentVariablesData_,
            tabulatedAtmosphereData.first.at( dependentVariableIndices_.at( 1 ) ),
            huntingAlgorithm,
            boundaryHandling_,
            defaultExtrapolationValue_.at( dependentVariableIndices_.at( 1 ) ) );
    interpolatorForTemperature_ = std::make_shared< MultiLinearInterpolator< double, double, NumberOfIndependentVariables > >(
            independentVariablesData_,
            tabulatedAtmosphereData.first.at( dependentVariableIndices_.at( 2 ) ),
            huntingAlgorithm,
            boundaryHandling_,
            defaultExtrapolationValue_.at( dependentVariableIndices_.at( 2 ) ) );

    // Create remaining interpolators, if requested by user
    if( dependentVariablesDependency_.at( 3 ) )
    {
        interpolatorForGasConstant_ = std::make_shared< MultiLinearInterpolator< double, double, NumberOfIndependentVariables > >(
                independentVariablesData_,
                tabulatedAtmosphereData.first.at( dependentVariableIndices_.at( 3 ) ),
                huntingAlgorithm,
                boundaryHandling_,
                defaultExtrapolationValue_.at( dependentVariableIndices_.at( 3 ) ) );
    }
    if( dependentVariablesDependency_.at( 4 ) )
    {
        interpolatorForSpecificHeatRatio_ = std::make_shared< MultiLinearInterpolator< double, double, NumberOfIndependentVariables > >(
                independentVariablesData_,
                tabulatedAtmosphereData.first.at( dependentVariableIndices_.at( 4 ) ),
                huntingAlgorithm,
                boundaryHandling_,
                defaultExtrapolationValue_.at( dependentVariableIndices_.at( 4 ) ) );
    }
    if( dependentVariablesDependency_.at( 5 ) )
    {
        interpolatorForMolarMass_ = std::make_shared< MultiLinearInterpolator< double, double, NumberOfIndependentVariables > >(
                independentVariablesData_,
                tabulatedAtmosphereData.first.at( dependentVariableIndices_.at( 5 ) ),
                huntingAlgorithm,
                boundaryHandling_,
                defaultExtrapolationValue_.at( dependentVariableIndices_.at( 5 ) ) );
    }
}
