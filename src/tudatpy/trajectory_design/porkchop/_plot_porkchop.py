...
def plot_porkchop_of_single_field(
    ...
    levels = np.logspace(
        np.log10(field_within_range.min()),
        np.log10(field_within_range.max()),
        number_of_levels,
    )
    ...
    if plot_isochrones:
        ...
        isochrones = plt.contour(
            x,
            y,
            (isoc - isoc.min()) / constants.JULIAN_DAY,
            colors="black",
            alpha=0.5,
            levels=10,
            linewidths=2,
    ...
    # Axis labels
    ax.set_xlabel("Departure date", fontsize=font_size + 3, weight="bold")
    ax.set_ylabel("Arrival date", fontsize=font_size + 3, weight="bold")
    ...
    ax.yaxis.set_ticks(y_ticks, [f"{tick_formatter(t)}" for t in y_ticks])
    ...
    if plot_minor_ticks:
        ...
        # Minor tick grid
        plt.grid(
            True, which="minor", linewidth=0.4, color="black", alpha=0.6, linestyle="--"
        )
