...
def trajectory_3d(
    ...
    for i, vehicle_name in enumerate(vehicles_names):
        if len(vehicle_name) != 0:
            ...
            min_pos, max_pos = min(min_pos, np.min(vehicles_positions[i])), max(
                max_pos, np.max(vehicles_positions[i])
            )
            ...
            ax.plot(
                ...
                vehicles_positions[i][:, 1],
                vehicles_positions[i][:, 2],
                label=vehicle_name,
                color=_color,
        ...
        body_state_array = np.array(
            [
                spice.get_body_cartesian_position_at_epoch(
                    spice_body, central_body_name, frame_orientation, "None", epoch
                )
                for epoch in sim_epochs
            ]
        )
        # Update the minimum and maximum positions
        min_pos, max_pos = min(min_pos, np.min(body_state_array)), max(
            max_pos, np.max(body_state_array)
        )
        # Select appropriate color and linestyle
        _color = colors[i_c]
        _linestyle = linestyles[i_ls]
    ...
    if center_plot:
        min_pos, max_pos = -max(np.fabs(min_pos), max_pos), max(
            np.fabs(min_pos), max_pos
        )

    # Add a legend, set the plot limits, and add axis labels
    ax.legend()
    ax.set_xlim([min_pos * 1.2, max_pos * 1.2]), ax.set_ylim(
        [min_pos * 1.2, max_pos * 1.2]
    ), ax.set_zlim([min_pos * 1.2, max_pos * 1.2])
...
def pareto_front(
    ...
    operator: List[Callable] = [min, min],
    alpha: float = 0.85,
):
    ...
    if c_parameter is None:
        ...
        fig, (ax, cax) = plt.subplots(
            figsize=(10, 6), ncols=2, gridspec_kw={"width_ratios": [1, 0.025]}
        )
    # Plot the two main objectives, with the appropriate color if the color-associated objective is specified
    objectives_scatter = ax.scatter(
        x_objective, y_objective, alpha=alpha, c=c_parameter, cmap=cmap
    )
    ...
    if c_parameter is not None:
        ...
        if c_label == "":
            print(
                "Warning: a colorbar has been added to the Pareto front plot, but no label was specified for it."
            )
    # Compute which of the points from the two main objectives are the Pareto optimums
    optimum_mask = pareto_optimums(
        np.array([x_objective, y_objective]).T, operator=operator
    )
    # Plot the pareto front (use the step function to not falsely indicate that a region is feasible without proof of it)
    ax.step(
        sorted(x_objective[optimum_mask], reverse=operator[0] == min),
        sorted(y_objective[optimum_mask], reverse=operator[1] == max),
        color="#418F3E",
        linewidth=2,
    )
