...
def pareto_optimums(_points: list, operator: Union[None, list[Callable]] = None):
    ...
    This function also allows to specify whether each dimension (which can be seen as an optimisation objective), should be minimised or maximised.
    ...
    Whether each of these points is a Pareto optimum is then computed, taking into account that an optimum is a minimum for x and y, and a maximum for z.
...
#
#     Creates a dictionary of state histories based on the unified `state_history`
#     from the propagation of multiple bodies. Each dictionary key contains the name of a propagated body,
...
#         Dictionary containing the name of the propagated body as key, and the state history as value.
#     """
...
def vector2matrix(flat_matrix: np.ndarray):
    ...
    This is detailed in the :func:`~tudatpy.dynamics.propagation_setup.dependent_variable.inertial_to_body_fixed_rotation_frame` docs.
...
def save2txt(solution, filename, directory="./"):
    ...
    df.index.name = "time"
    df.to_csv(os.path.join(directory, _filename), header=False, sep="\t")
