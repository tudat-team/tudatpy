...
class DerivedDopplerConverter(RadioBase):
    """Converter for derived Doppler TNF records."""

    def extract(self, sfdu_list: list[SFDU]) -> DataFrame:
        """Extract derived Doppler records from SFDU objects.

        Parameters
        ----------
        sfdu_list : list[trk234.SFDU]
            SFDU records to inspect.

        Returns
        -------
        pandas.DataFrame
            Extracted derived Doppler records.
        """
    ...
    def process(
        ...
        """Convert derived Doppler records to tracking-data objects.

        Parameters
        ----------
        doppler_df : pandas.DataFrame
            Extracted derived Doppler records.
        spacecraftName : str, optional
            Optional spacecraft body name.
