...
class DerivedSraRangeConverter(RadioBase):
    """Converter for derived SRA range TNF records."""

    def extract(self, sfdu_list: list[SFDU]) -> DataFrame:
        """Extract derived SRA range records from SFDU objects.
        ...
        -------
        pandas.DataFrame
            Extracted derived SRA range records.
        """
    ...
    def process(self, range_df: DataFrame, spacecraftName: str | None = None) -> list[TrackingData]:
        """Convert derived SRA range records to tracking-data objects.
