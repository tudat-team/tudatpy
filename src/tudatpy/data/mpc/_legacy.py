"""Behavioral compatibility for pre-refactor MPC scripts.

The legacy batch owns environment setup, weights, filtering and plotting as well
as ingestion. Retain that implementation during deprecation; the data_input
BatchMPC remains independent. Ported from develop at 6dfa06e689.
"""
...
class BatchMPC:
    ...
    def _add_time_columns(self, table: pd.DataFrame) -> pd.DataFrame:
        ...
        augmented_table["epoch_seconds_UTC"] = [dt_obj.to_epoch() for dt_obj in dt_objects]
    ...
    def create_observations_from_astropy_table(
        ...
        for (mpc_code, observatory_code), group in unique_links:
            ...
            link_ends[links.transmitter] = links.body_origin_link_end_id(
                _resolve_optical_target_names(temp_batch.table)[mpc_code]
            )
    ...
    def to_tudat(
                ...
                if add_sbdb_gravity_model:
                    add_gravity_field_model(
                        ...
                        central_sbdb(
                            next(
                                code
                                for code, name in _resolve_optical_target_names(self._table).items()
                                if name == body
                            )
                        ),
                    )
        ...
        for combo in unique_link_combos:
            ...
            # observed body link
            link_ends[links.transmitter] = links.body_origin_link_end_id(
                _resolve_optical_target_names(self._table)[MPC_number]
            )
