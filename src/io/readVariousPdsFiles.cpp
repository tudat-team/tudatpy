...
double computeTimeTdbFromUtc( double timeUtc )
{
    double timeTdb = earth_orientation::TerrestrialTimeScaleConverter( ).getCurrentTimes(
            basic_astrodynamics::utc_scale, basic_astrodynamics::tdb_scale, std::vector< double >( { timeUtc } ), Eigen::Vector3d::Zero( ) ).at( 0 );
    return timeTdb;
}

std::map< double, Eigen::Vector3d > grailAntennaFileReader( const std::string antennaFile )
{
    std::map< double, Eigen::Vector3d > antennaSwitchHistory;
    ...
    while ( dataFile.good( ) && !dataFile.eof( ) )
    {
        ...
        if ( isHeaderParsed )
        {
            ...
            if ( !emptyFlags.compare( "00000000" ) )
            {
            ...
            antennaSwitchHistory[ switchTimeTbd ] = antennaPosition;
        }
...
//            }
            ...
            double timeInSecondsTdb = computeTimeTdbFromUtc( timeInSecondsUtc );
    ...
    while ( dataFile.good( ) && !dataFile.eof( ) )
    {
        ...
        if ( isHeaderParsed )
        {
            ...
            if ( dataLevel == "1a" )
            {
               ...
               timeTdb = computeTimeTdbFromUtc( timeUtcScet );
            }
            if ( dataLevel == "1b" )
            {
