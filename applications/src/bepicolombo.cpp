...
//    const auto simulationDuration = 2 * 8355.0; // 2 orbits
    const auto simulationDuration = 1 * 365 * 24 * 3600; // 13 years
...
SystemOfBodies createSimulationBodies()
{
    ...
    bodySettings.at("Mercury")->gravityFieldSettings = sphericalHarmonicsGravitySettings(
                3.3e23 * GRAVITATIONAL_CONSTANT, 2439e3, cosineCoefficients, sineCoefficients, mercuryFrame );
...
Eigen::VectorXd createSimulationInitialState()
{
    ...
    auto initialState = ephemerisMPO->getCartesianState(simulationStartEpoch);
    return initialState;
}
