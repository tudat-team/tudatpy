...
int main( )
{
    std::string outputDirectory = "/home/dominic/Documents/Courses/NumericalAstrodynamics2021/Lectures/Week2/EmpiricalAccelerationExample/";
    ...
    BodyListSettings bodySettings = getDefaultBodySettings( bodiesToCreate, "Moon", "ECLIPJ2000" );
    ...
    for( unsigned int eccentricityCase = 0; eccentricityCase < 2; eccentricityCase++ )
    {
        for( unsigned int propagationCase = 0; propagationCase < 10; propagationCase++ )
        {
            ...
            accelerationsOfLunarOrbiter[ "Moon" ].push_back(
            ...
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back(
                        std::make_shared< EmpiricalAccelerationSettings >( empiricalAccelerationNorm * Eigen::Vector3d::UnitY( ) ) );
            }
            else if( propagationCase == 3 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back(
                        std::make_shared< EmpiricalAccelerationSettings >( empiricalAccelerationNorm * Eigen::Vector3d::UnitZ( ) ) );
            }
            else if( propagationCase == 4 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back( std::make_shared< EmpiricalAccelerationSettings >(
                        Eigen::Vector3d::Zero( ), empiricalAccelerationNorm * Eigen::Vector3d::UnitX( ) ) );
            }
            else if( propagationCase == 5 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back( std::make_shared< EmpiricalAccelerationSettings >(
                        Eigen::Vector3d::Zero( ), empiricalAccelerationNorm * Eigen::Vector3d::UnitY( ) ) );
            }
            else if( propagationCase == 6 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back( std::make_shared< EmpiricalAccelerationSettings >(
                        Eigen::Vector3d::Zero( ), empiricalAccelerationNorm * Eigen::Vector3d::UnitZ( ) ) );
            }
            else if( propagationCase == 7 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back( std::make_shared< EmpiricalAccelerationSettings >(
                        Eigen::Vector3d::Zero( ), Eigen::Vector3d::Zero( ), empiricalAccelerationNorm * Eigen::Vector3d::UnitX( ) ) );
            }
            else if( propagationCase == 8 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back( std::make_shared< EmpiricalAccelerationSettings >(
                        Eigen::Vector3d::Zero( ), Eigen::Vector3d::Zero( ), empiricalAccelerationNorm * Eigen::Vector3d::UnitY( ) ) );
            }
            else if( propagationCase == 9 )
            {
                accelerationsOfLunarOrbiter[ "Moon" ].push_back( std::make_shared< EmpiricalAccelerationSettings >(
                        Eigen::Vector3d::Zero( ), Eigen::Vector3d::Zero( ), empiricalAccelerationNorm * Eigen::Vector3d::UnitZ( ) ) );
            }
            ...
            lunarOrbiterInitialStateInKeplerianElements( argumentOfPeriapsisIndex ) = convertDegreesToRadians( 235.7 );
            lunarOrbiterInitialStateInKeplerianElements( longitudeOfAscendingNodeIndex ) = convertDegreesToRadians( 23.4 );
            ...
            Eigen::VectorXd systemInitialState =
                    convertKeplerianToCartesianElements( lunarOrbiterInitialStateInKeplerianElements, moonGravitationalParameter );

            // Define list of dependent variables to save.
            std::vector< std::shared_ptr< SingleDependentVariableSaveSettings > > dependentVariablesList;
            dependentVariablesList.push_back(
                    std::make_shared< SingleDependentVariableSaveSettings >( keplerian_state_dependent_variable, "LunarOrbiter", "Moon" ) );
