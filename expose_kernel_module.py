...
raise DeprecationWarning("This script is deprecated!")
...
def create_import_statement_to_import_all_module_members(module):
    ...
    if module_methods(module):
        import_statement += [
            ...
            f"     {name}" + (", \\" if i != len(module_methods(module)) - 1 else "")
            for i, (name, _) in enumerate(module_methods(module))
        ]
    # Import objects
    if module_objects(module):
        import_statement += [
            ...
            f"     {name}" + (", \\" if i != len(module_objects(module)) - 1 else "")
            for i, (name, _) in enumerate(module_objects(module))
        ]
...
"""
    ...
    kernel_member_import_file_path = (
        hybrid_module_path / "_import_all_kernel_members.py"
    )
...
explanation_of_module_exposure_process = (
    lambda module_name: f"""# This file, by virtue of the import statement below, merges
...
def set_up__init__(module):
    ...
    import_statement = "\n".join(
        ...
        + (
            ...
            if create_import_statement_to_import_all_module_members(module)
            else []
        )
    )
...
sys.path.insert(
    0,
    f"/mnt/e/studio/professional/work/2023-2024 Tudat/repos/tudat-bundle/{args.build_dir}/tudatpy/",
)
