...
std::vector< double > EarthMarsTransfer::fitness( const std::vector< double > &xv ) const
{
    ...
    MultiRevolutionLambertTargeterIzzo lambertTargeter( initialState.segment( 0, 3 ), finalState.segment( 0, 3 ), xv[ 1 ] * 86400, mu );
    ...
    for( unsigned int i = 0; i <= maxrev; ++i )
    {
        ...
        deltaV = std::min( deltaV,
                           ( initialState.segment( 3, 3 ) - lambertTargeter.getInertialVelocityAtDeparture( ) ).norm( ) +
                                   +( finalState.segment( 3, 3 ) - lambertTargeter.getInertialVelocityAtArrival( ) ).norm( ) );
    }
...
EarthMarsTransfer::StateType EarthMarsTransfer::getPlanetPosition( const double date, const std::string planetName ) const
{
    ...
    stateKepl( 5 ) = convertMeanAnomalyToEccentricAnomaly(
            stateKepl( 1 ), fmod( stateKepl( 5 ) + ( date - jd0 ) * 86400. * n, 2. * boost::math::constants::pi< double >( ) ) );
