...
void MultipleGravityAssist::getDecomposedDecisionVector( const Eigen::VectorXd rawDecisionVariables,
                                                         ...
                                                         std::vector< Eigen::VectorXd >& currentNodeFreeParameters ) const
{
    ...
    {
        currentNodeTimes.push_back( currentNodeTimes.at( i - 1 ) + rawDecisionVariables( i ) * JD );
    }

    for( unsigned int i = 0; i < legParameterIndices_.size( ); i++ )
    {
        currentLegFreeParameters.push_back(
                rawDecisionVariables.segment( legParameterIndices_.at( i ).first, legParameterIndices_.at( i ).second ) );
    }

    for( unsigned int i = 0; i < nodeParameterIndices_.size( ); i++ )
    {
        currentNodeFreeParameters.push_back(
                rawDecisionVariables.segment( nodeParameterIndices_.at( i ).first, nodeParameterIndices_.at( i ).second ) );
    }
}
...
std::vector< double > MultipleGravityAssist::fitness( const std::vector< double >& xv ) const
{
    ...
    getDecomposedDecisionVector( tudat::utilities::convertStlVectorToEigenVector( xv ),
                                 currentNodeTimes_,
                                 currentLegFreeParameters_,
                                 currentNodeFreeParameters_ );
    if( transferTrajectory_ != nullptr )
    {
        transferTrajectory_->evaluateTrajectory( currentNodeTimes_, currentLegFreeParameters_, currentNodeFreeParameters_ );
    }
    else
    {
        transferTrajectory_ = createTransferTrajectory( bodyMap_,
                                                        legSettings_,
                                                        nodeSettings_,
                                                        nodeIds_,
                                                        centralBody_,
