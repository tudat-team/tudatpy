        ...
        const double time )
{
    ...
    Eigen::Matrix< double, 1, Eigen::Dynamic > partialVector =
            Eigen::Matrix< double, 1, Eigen::Dynamic >::Zero( 1, polynomialPowers_.size( ) );
        ...
        const double time )
{
    ...
    Eigen::Matrix< double, 1, Eigen::Dynamic > partialVector =
            Eigen::Matrix< double, 1, Eigen::Dynamic >::Zero( 1, numberOfArcs_ * polynomialPowers_.size( ) );
    ...
    if( arcFindIterator_ != arcIndices_.end( ) )
    {
        ...
        {
            partialVector( 0, i ) =
                    std::pow( static_cast< double >( timeIntoCurrentArcAndArcIndex.first ), polynomialPowers_[ i - startIndex ] );
        }
    }
    return partialVector;
}
