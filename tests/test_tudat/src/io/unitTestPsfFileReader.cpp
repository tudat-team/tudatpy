...
BOOST_AUTO_TEST_CASE( testSinglePsfFileReader )
{
    ...
    const input_output::psf::RawPsfFileContents psfFile = input_output::psf::readRawPsfFile( file );
    ...
    BOOST_REQUIRE_EQUAL( psfFile.trackingSupplementaryData_.size( ), 2 );
    std::map< std::string, std::shared_ptr< data::TrackingSupplementaryData > > cameraSupplementaryData;
    for( const std::shared_ptr< data::TrackingSupplementaryData >& supplementaryData : psfFile.trackingSupplementaryData_ )
    {
        BOOST_REQUIRE( supplementaryData != nullptr );
        BOOST_CHECK_EQUAL( supplementaryData->getBodyName( ), "VGR2" );
        cameraSupplementaryData[ supplementaryData->getReferencePointName( ) ] = supplementaryData;
    }
    BOOST_REQUIRE( cameraSupplementaryData.count( "A" ) == 1 );
    BOOST_REQUIRE( cameraSupplementaryData.count( "B" ) == 1 );

    const std::shared_ptr< const data::CameraInstrumentSupplementaryData > cameraA =
            input_output::psf::getPsfCameraInstrumentSupplementaryData( psfFile, "A" );
    BOOST_REQUIRE( cameraA != nullptr );
    BOOST_CHECK_EQUAL( cameraA->getCameraId( ), "A" );
    checkClose( cameraA->getFocalLength( ), 1503.4900 );
    checkVector2Close( cameraA->getPrincipalPoint( ), 398.030, 399.390 );
    checkClose( cameraA->getFieldOfViewBounds( )( 0 ), 0.000 );
    checkClose( cameraA->getFieldOfViewBounds( )( 1 ), 800.000 );
    checkClose( cameraA->getFieldOfViewBounds( )( 2 ), 0.000 );
    checkClose( cameraA->getFieldOfViewBounds( )( 3 ), 800.000 );
    checkClose( cameraA->getKMatrix( )( 0, 0 ), 7.1808000E+01 );
    checkClose( cameraA->getKMatrix( )( 1, 0 ), -1.0938000E+00 );
    checkClose( cameraA->getKMatrix( )( 0, 1 ), 7.6998000E-01 );
    checkClose( cameraA->getKMatrix( )( 1, 1 ), 7.2951000E+01 );
    checkClose( cameraA->getKMatrix( )( 0, 2 ), -9.3650000E-03 );
    checkClose( cameraA->getKMatrix( )( 1, 2 ), 6.5570000E-03 );

    const std::shared_ptr< const data::CameraInstrumentSupplementaryData > cameraB =
            input_output::psf::getPsfCameraInstrumentSupplementaryData( psfFile, "B" );
    BOOST_REQUIRE( cameraB != nullptr );
    BOOST_CHECK_EQUAL( cameraB->getCameraId( ), "B" );
    checkClose( cameraB->getFocalLength( ), 200.7260 );
    checkVector2Close( cameraB->getPrincipalPoint( ), 402.631, 404.101 );
    checkClose( cameraB->getMountingOffsets( )( 0 ), -3.0845000E-02 );
    checkClose( cameraB->getMountingOffsets( )( 1 ), -6.7920000E-03 );
    checkClose( cameraB->getMountingOffsets( )( 2 ), 1.7111900E-01 );
    ...
    const auto trackingDataAndSupplementaryData = input_output::psf::readPsfFile< double, double >(
            file, "VGR2", std::map< std::string, std::string >( { { "TRITON", "TRITON" } } ), false );
    BOOST_REQUIRE( !trackingDataAndSupplementaryData.first.empty( ) );
    BOOST_REQUIRE_EQUAL( trackingDataAndSupplementaryData.second.size( ), 2 );
    BOOST_CHECK_EQUAL( trackingDataAndSupplementaryData.first.front( )->getObservableType( ), "PixelCoordinates" );
    BOOST_CHECK_EQUAL( trackingDataAndSupplementaryData.first.front( )->getReferenceLinkEnd( ), "receiver" );
    BOOST_CHECK_EQUAL( trackingDataAndSupplementaryData.first.front( )->getTimeScale( ), "UTC" );
