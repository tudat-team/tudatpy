...
// #include "ComaSettings.h" // Replace with the actual path to your header
...
BOOST_AUTO_TEST_CASE( testComaSettingsSingleFile )
{
    ...
    // Access parsed data
    const auto& polyCoefs = comaSettings.getPolyCoefficients( );
    const auto& SHDegreeAndOrder = comaSettings.getSHDegreeAndOrder( );
    const auto& referenceRadius = comaSettings.getReferenceRadius( );
    const auto& powerInvRadius = comaSettings.getPowersInvRadius( );
    const auto& maxDegree = comaSettings.getRequestedDegree( );
    const auto& maxOrder = comaSettings.getRequestedOrder( );

    // Check polyCoefs
    BOOST_REQUIRE_EQUAL( polyCoefs.size(), 1 );
    // Rows and Columns are flipped internally
    BOOST_CHECK_EQUAL( polyCoefs[0].rows(), 48 );
    BOOST_CHECK_EQUAL( polyCoefs[0].cols(), 121 );

    BOOST_CHECK_CLOSE( polyCoefs[0](0, 0), 6.262302500423528E+02, 1.0e-12 );
    BOOST_CHECK_CLOSE( polyCoefs[0](3, 1), -4.459813047951577E-02, 1.0e-12 );
    BOOST_CHECK_CLOSE( polyCoefs[0](47, 120), -1.049515320967208E+02, 1.0e-12 );
    BOOST_CHECK_CLOSE( polyCoefs[0](10, 22), 1.287417812579956E-01, 1.0e-12 );

    // Check SHDegreeAndOrder
    BOOST_REQUIRE_EQUAL( SHDegreeAndOrder.size(), 1 );
    BOOST_REQUIRE_EQUAL( SHDegreeAndOrder[0].rows(), 2 );
    BOOST_REQUIRE_EQUAL( SHDegreeAndOrder[0].cols(), 121 );
    BOOST_REQUIRE_EQUAL( SHDegreeAndOrder[0](0, 120), 10 ); // First row, last column
    BOOST_REQUIRE_EQUAL( SHDegreeAndOrder[0](1, 120), -10 ); // Second row, last column

    // Check reference radius
    BOOST_CHECK_EQUAL( referenceRadius[0], 10.0 );

    // Check power inv radius
    BOOST_CHECK_EQUAL( powerInvRadius.size(), 1 );
    BOOST_CHECK_EQUAL( powerInvRadius[0].rows(), 4 );
    BOOST_CHECK_EQUAL( powerInvRadius[0][0], 0 );
    BOOST_CHECK_EQUAL( powerInvRadius[0][3], 3 );

    // Check max degree and order
    BOOST_CHECK_EQUAL( maxDegree, 10 );
    BOOST_CHECK_EQUAL( maxOrder, 10 );
