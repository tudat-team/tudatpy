...
BOOST_FIXTURE_TEST_CASE( test_wind_processor_construction, WindTestDataPaths )
{
    ...
    BOOST_CHECK_NO_THROW( ComaWindModelFileProcessor processor( xFiles, yFiles, zFiles ) );
...
BOOST_FIXTURE_TEST_CASE( test_wind_processor_create_sh_datasets_from_poly, WindTestDataPaths )
{
    ...
    ComaWindDatasetCollection windDatasets =
            processor.createSHDatasets( radii_m, solLongitudes_deg, ExpectedWindValues::maxDegree, ExpectedWindValues::maxOrder );
...
BOOST_FIXTURE_TEST_CASE( test_wind_processor_create_sh_files, WindTestDataPaths )
{
    ...
    processor.createSHFiles(
            xOutputDir.string( ), yOutputDir.string( ), zOutputDir.string( ), radii_m, solLongitudes_deg, 8, 6  // max degree and order
    );
...
BOOST_FIXTURE_TEST_CASE( test_wind_processor_poly_to_stokes, WindTestDataPaths )
{
    ...
    ComaWindDatasetCollection windDatasetsAllZ =
            processorAllZ.createSHDatasets( radii_m, solLongitudes_deg, ExpectedWindValues::maxDegree, ExpectedWindValues::maxOrder );
...
BOOST_FIXTURE_TEST_CASE( test_wind_processor_error_handling, WindTestDataPaths )
{
    ...
    BOOST_CHECK_THROW( processor.createSHDatasets( ), std::runtime_error );
    ...
    ComaWindModelFileProcessor stokesProcessor( xOutputDir.string( ), yOutputDir.string( ), zOutputDir.string( ) );
    ...
    BOOST_CHECK_THROW( stokesProcessor.createPolyCoefDatasets( ), std::runtime_error );

    // Should throw when trying to create SH datasets with parameters
    BOOST_CHECK_THROW( stokesProcessor.createSHDatasets( { 1000.0 }, { 45.0 } ), std::runtime_error );

    // But should work without parameters
    BOOST_CHECK_NO_THROW( stokesProcessor.createSHDatasets( ) );
}
...
BOOST_FIXTURE_TEST_CASE( test_wind_model_velocity_computation, WindTestDataPaths )
{
    ...
    std::cout << "\n=== Part A.1: Testing sol=0°, r=4km ===" << std::endl;
    {
        ...
        auto comaModel = std::make_shared< aerodynamics::ComaModel >( zStokes,
                                                                      molecularWeight,
                                                                      sunStateFunction,
                                                                      cometStateFunction,
        ...
        aerodynamics::ComaWindModel windModel( xStokes,
                                               yStokes,
                                               zStokes,
                                               comaModel,
                                               sunStateFunction,
        ...
        for( size_t i = 0; i < residual_0deg_4km.points.size( ); i += numSampled )
        {
            ...
            // Call wind model
            Eigen::Vector3d windVelocity = windModel.getCurrentBodyFixedCartesianWindVelocity( testRadius, lon_rad, lat_rad, testTime );
    ...
    std::cout << "\n=== Part B: Testing all components using Z data ===" << std::endl;
    {
        ...
        aerodynamics::ComaWindModel windModelAllZ( zStokes,  // Use Z for X component
        ...
        for( size_t i = 0; i < residual_0deg_4km.points.size( ); i += numSampled )
        {
            ...
            Eigen::Vector3d windVelocity = windModelAllZ.getCurrentBodyFixedCartesianWindVelocity( testRadius, lon_rad, lat_rad, testTime );
...
BOOST_FIXTURE_TEST_CASE( test_wind_model_velocity_validation_from_python, WindTestDataPaths )
{
    ...
    while( std::getline( file, line ) )
    {
        ...
        if( iss >> point.time >> point.radialDistance >> point.latitude >> point.longitude >> point.solarLongitude >> point.density >>
            point.windX >> point.windY >> point.windZ )
        {
    ...
    for( int idx : selectedIndices )
    {
        ...
        auto comaModel = std::make_shared< aerodynamics::ComaModel >(
                densityStokes, molecularWeight, sunStateFunction, cometStateFunction, cometRotationFunction, maxDegree, maxOrder );
        ...
        const Eigen::Vector3d computedWindVelocity =
                windModel.getCurrentBodyFixedCartesianWindVelocity( point.radialDistance, point.longitude, point.latitude, point.time );
            ...
            if( failCount <= 10 )  // Only report first 10 failures
            {
                BOOST_TEST_MESSAGE(
                        "Point " << idx << " mismatch: "
                                 ...
                                 << "computed=(" << computedWindVelocity[ 0 ] << ", " << computedWindVelocity[ 1 ] << ", "
                                 << computedWindVelocity[ 2 ] << ") m/s, "
        ...
        BOOST_CHECK_MESSAGE( withinToleranceX,
                             "Wind X mismatch at r=" << point.radialDistance << " m, "
                                                     << "lat=" << point.latitude * 180.0 / PI << "°, "
                                                     << "lon=" << point.longitude * 180.0 / PI << "°"
                                                     << ", abs_error=" << absoluteErrorX << ", rel_error=" << relativeErrorX );
        BOOST_CHECK_MESSAGE( withinToleranceY,
                             "Wind Y mismatch at r=" << point.radialDistance << " m, "
                                                     << "lat=" << point.latitude * 180.0 / PI << "°, "
                                                     << "lon=" << point.longitude * 180.0 / PI << "°"
                                                     << ", abs_error=" << absoluteErrorY << ", rel_error=" << relativeErrorY );
        BOOST_CHECK_MESSAGE( withinToleranceZ,
                             "Wind Z mismatch at r=" << point.radialDistance << " m, "
                                                     << "lat=" << point.latitude * 180.0 / PI << "°, "
                                                     << "lon=" << point.longitude * 180.0 / PI << "°"
                                                     << ", abs_error=" << absoluteErrorZ << ", rel_error=" << relativeErrorZ );
    }

    BOOST_TEST_MESSAGE( "Python wind reference validation: Verified " << selectedIndices.size( ) << " randomly selected points out of "
