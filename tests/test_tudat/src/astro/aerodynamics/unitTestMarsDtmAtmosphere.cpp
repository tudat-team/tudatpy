...
BOOST_AUTO_TEST_CASE( testMarsDtmAtmosphere )
{
    ...
    std::shared_ptr< aerodynamics::AtmosphereModel > marsAtmosphereModel =
            createAtmosphereModel( marsDtmAtmosphereSettings, "Mars", bodies );
    std::shared_ptr< MarsDtmAtmosphereModel > atmosphereModel =
            std::dynamic_pointer_cast< MarsDtmAtmosphereModel >( createAtmosphereModel( marsDtmAtmosphereSettings, "Mars", bodies ) );
    int alt_km = 400E3;
    int time = 86400;
...
BOOST_AUTO_TEST_CASE( testMarsDtmAtmosphereInPropagation )
{
    ...
    dependentVariables.push_back(
            std::make_shared< SingleAccelerationDependentVariableSaveSettings >( aerodynamic, "Vehicle", "Mars", 0 ) );
