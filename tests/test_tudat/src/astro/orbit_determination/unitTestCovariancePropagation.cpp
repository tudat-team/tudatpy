...
BOOST_AUTO_TEST_CASE( test_CovariancePropagationReferenceEpoch )
{
    ...
    for( int referenceEpochTest = 0; referenceEpochTest < 2; referenceEpochTest++ )
    {
        ...
        {
            systemInitialState = convertKeplerianToCartesianElements( asterixInitialStateInKeplerianElements, earthGravitationalParameter );
        }
        else
        {
    ...
    while (itStart != propagatedCovarianceStartReference.end())
    {
        ...
        for (int rowBlockIndex = 0; rowBlockIndex < 2; ++rowBlockIndex)
        {
            ...
            for (int columnBlockIndex = 0; columnBlockIndex < 2; ++columnBlockIndex)
            {
                ...
                Eigen::MatrixXd blockStart  = covarianceMatrixStart.block(rowStartIndex, columnStartIndex,
                                                                                numberOfRowsInBlock, numberOfColumnsInBlock);
                Eigen::MatrixXd blockMiddle = covarianceMatrixMiddle.block(rowStartIndex, columnStartIndex,
                                                                                  numberOfRowsInBlock, numberOfColumnsInBlock);
