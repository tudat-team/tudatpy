                                                                      ...
                                                                      const std::vector< std::string >& centralBodyNames )
{
    ...
    {
        accelerationSettingsJuice[ bodyNames.at( i ) ][ "Jupiter" ].push_back(
        ...
        const SystemOfBodies& bodies )
{
    ...
    std::vector< std::shared_ptr< EstimatableParameterSettings > > parameterNames =
            getInitialStateParameterSettings< double, double >( hybridArcPropagatorSettings, bodies );
...
BOOST_AUTO_TEST_CASE( testPropagatorParameterConsistency )
{
    ...
    for( unsigned int i = 0; i < flybysBodies.size( ); i++ )
    {
        ...
        {
            initialStates.segment( 6 * j, 6 ) = spice_interface::getBodyCartesianStateAtEpoch(
                                                        "JUICE", centralBodies.at( j ), "ECLIPJ2000", "NONE", flybysTimes.at( i ) ) +
    ...
    for( int i = 0; i < 4; i++ )
    {
        initialStates.segment( 6 * i, 6 ) = spice_interface::getBodyCartesianStateAtEpoch(
                galileanSatelliteNames.at( i ), "Jupiter", "ECLIPJ2000", "NONE", flybysTimes.at( 0 ) );
    }
    ...
    std::shared_ptr< HybridArcPropagatorSettings< double > > hybridArcPropagatorSettings =
            std::make_shared< HybridArcPropagatorSettings< double > >( singleArcPropagatorSettings, multiArcPropagatorSettings );

    std::shared_ptr< estimatable_parameters::EstimatableParameterSet< double > > hybridArcParameters =
            getParametersToEstimate( hybridArcPropagatorSettings, bodies );
    ...
    for( int singleArcTest = 0; singleArcTest < 3; singleArcTest++ )
    {
        ...
        for( int singleArcBody = 0; singleArcBody < 4; singleArcBody++ )
        {
            ...
            Eigen::Vector6d stateDifference = singleArcPropagatorSettings->getInitialStates( ).segment( startIndex, 6 ) -
                    originalInitialStates.segment( startIndex, 6 );
            TUDAT_CHECK_MATRIX_CLOSE_FRACTION( stateDifference, statePerturbation, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
            TUDAT_CHECK_MATRIX_CLOSE_FRACTION( ( singleArcPropagatorSettings->getInitialStates( ).segment( 0, 24 ) ),
    ...
    for( unsigned int i = 0; i < hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )->getSingleArcSettings( ).size( ); i++ )
    {
        ...
        currentIndex +=
                hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )->getSingleArcSettings( ).at( i )->getConventionalStateSize( );
    }
    ...
    for( int multiArcTest = 0; multiArcTest < 4; multiArcTest++ )
    {
        ...
        for( int bodyIndex = 0; bodyIndex < 2; bodyIndex++ )
        {
            ...
            for( int arcIndex = 0; arcIndex < 4; arcIndex++ )
            {
                ...
                Eigen::VectorXd originalValueFromSingleArc = hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )
                                                                     ->getSingleArcSettings( )
                                                                     .at( propagatorArcIndex )
                                                                     ->getInitialStates( )
                                                                     .segment( singleArcPropagatorStartIndex, 6 );
                Eigen::VectorXd originalValueFromHybridArc =
                        hybridArcPropagatorSettings->getInitialStates( ).segment( hybridArcPropagatorStartIndex, 6 );
                Eigen::VectorXd originalValueFromMultiArc =
                        multiArcPropagatorSettings->getInitialStates( ).segment( hybridArcPropagatorStartIndex - 24, 6 );

                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        originalValueFromParameter, originalValueFromSingleArc, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        originalValueFromParameter, originalValueFromMultiArc, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        originalValueFromParameter, originalValueFromHybridArc, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                ...
                {
                    Eigen::VectorXd perturbedSingleArcValue = hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )
                                                                      ->getSingleArcSettings( )
                                                                      .at( propagatorArcIndex )
                                                                      ->getInitialStates( );
                    ...
                    hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )
                            ->getSingleArcSettings( )
                            .at( propagatorArcIndex )
                            ->resetInitialStates( perturbedSingleArcValue );
                }
                else if( multiArcTest == 2 )
                {
                    Eigen::VectorXd perturbedMultiArcValue =
                            hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )->getInitialStates( );
                ...
                Eigen::VectorXd perturbedValueFromSingleArc = hybridArcPropagatorSettings->getMultiArcPropagatorSettings( )
                                                                      ->getSingleArcSettings( )
                                                                      .at( propagatorArcIndex )
                                                                      ->getInitialStates( )
                                                                      .segment( singleArcPropagatorStartIndex, 6 );
                Eigen::VectorXd perturbedValueFromHybridArc =
                        hybridArcPropagatorSettings->getInitialStates( ).segment( hybridArcPropagatorStartIndex, 6 );
                Eigen::VectorXd perturbedValueFromMultiArc =
                        multiArcPropagatorSettings->getInitialStates( ).segment( hybridArcPropagatorStartIndex - 24, 6 );
                ...
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        stateDifference, statePerturbation, ( 10.0 * std::numeric_limits< double >::epsilon( ) ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        perturbedValueFromParameter, perturbedValueFromSingleArc, ( std::numeric_limits< double >::epsilon( ) ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        perturbedValueFromParameter, perturbedValueFromMultiArc, ( std::numeric_limits< double >::epsilon( ) ) );
                TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                        perturbedValueFromParameter, perturbedValueFromHybridArc, ( std::numeric_limits< double >::epsilon( ) ) );
            }
        }
    }
}
