                                               ...
                                               const double observationTime )
{
    ...
    Eigen::VectorXd nominalObservable =
            oneWayDopplerModel->computeIdealObservationsWithLinkEndData( observationTime, referenceLinkEnd, linkEndTimes, linkEndStates );
...
BOOST_AUTO_TEST_CASE( testOneWayDopplerPartials )
{
    ...
    for( unsigned int normalizeObservable = 0; normalizeObservable < 2; normalizeObservable++ )
    {
        ...
        {
            std::cout << "ESTIMATION CASE *********************************** " << normalizeObservable << " " << estimationCase
                      << std::endl;
        ...
        // Test partials with constant ephemerides (allows test of position partials)
        {
            ...
            TUDAT_CHECK_MATRIX_CLOSE_FRACTION( ( ( partialWrtMarsState - partialWrtMarsStateWithoutProperTime ).block( 0, 0, 1, 3 ) ),
                                               ( properTimePartialWrtMarsPosition + properTimePartialWrtEarthPosition ),
                                               1.0E-5 );
