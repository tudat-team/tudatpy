...
double computeNormalizedEffectiveMultiplierFromLiterature( const int degree1, const int order1, const int degree2, const int order2 )
{
    ...
    return gamma * std::sqrt( 4.0 * mathematical_constants::PI * deltaOrder1 * deltaOrder2 / deltaCombinedOrder ) * deltaOrder1 *
            deltaOrder2 * parityFactor;
}
                                                                                ...
                                                                                const int order2 )
{
    ...
    const double expectedCosine = ( cosineBody1 * cosineBody2 - sineBody1 * sineBody2 ) * multiplier;
    const double expectedSine = ( sineBody1 * cosineBody2 + cosineBody1 * sineBody2 ) * multiplier;
