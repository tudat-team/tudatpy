...
//! Test wrapping ranges for position-angle and separation-distance observables.
BOOST_AUTO_TEST_CASE( testPositionAngleAndSeparationWrappingRanges )
{
    const std::vector< ResidualWrappingRange > positionAngleRanges = getResidualWrappingRanges( position_angle );
    BOOST_REQUIRE_EQUAL( positionAngleRanges.size( ), 1 );
    BOOST_CHECK_CLOSE( positionAngleRanges[ 0 ].minimumRange, -PI, 1.0e-15 );
    BOOST_CHECK_CLOSE( positionAngleRanges[ 0 ].maximumRange, PI, 1.0e-15 );

    const std::vector< ResidualWrappingRange > positionAngleAndSeparationRanges =
            getResidualWrappingRanges( position_angle_and_separation );
    BOOST_REQUIRE_EQUAL( positionAngleAndSeparationRanges.size( ), 2 );
    BOOST_CHECK_CLOSE( positionAngleAndSeparationRanges[ 0 ].minimumRange, -PI, 1.0e-15 );
    BOOST_CHECK_CLOSE( positionAngleAndSeparationRanges[ 0 ].maximumRange, PI, 1.0e-15 );
    BOOST_CHECK_SMALL( positionAngleAndSeparationRanges[ 1 ].period( ), 1.0e-15 );

    BOOST_CHECK( getResidualWrappingRanges( separation_distance ).empty( ) );
}
...
//! Test wrapping of the standalone and combined position-angle observables.
BOOST_AUTO_TEST_CASE( testWrappingOfPositionAngleResiduals )
{
    Eigen::VectorXd positionAngleResiduals( 5 );
    positionAngleResiduals << 3.0 * PI, PI, -3.0 * PI, -PI, 0.25 * PI;

    simulation_setup::wrapObservationResiduals< double >( positionAngleResiduals, std::make_pair( 0, 5 ), position_angle );

    const Eigen::VectorXd expectedPositionAngleResiduals = ( Eigen::VectorXd( 5 ) << -PI, PI, PI, -PI, 0.25 * PI ).finished( );
    for( int i = 0; i < positionAngleResiduals.rows( ); i++ )
    {
        BOOST_CHECK_SMALL( positionAngleResiduals( i ) - expectedPositionAngleResiduals( i ), 1.0e-14 );
    }

    Eigen::VectorXd combinedResiduals( 10 );
    combinedResiduals << 42.0, 3.0 * PI, 0.75 * PI, -3.0 * PI, -0.75 * PI, 10.0 * PI, 4.0 * PI, 0.5 * PI, -4.0 * PI, -42.0;

    simulation_setup::wrapObservationResiduals< double >( combinedResiduals, std::make_pair( 1, 8 ), position_angle_and_separation );

    const Eigen::VectorXd expectedCombinedResiduals =
            ( Eigen::VectorXd( 10 ) << 42.0, -PI, 0.75 * PI, PI, -0.75 * PI, 0.0, 4.0 * PI, 0.5 * PI, -4.0 * PI, -42.0 ).finished( );
    for( int i = 0; i < combinedResiduals.rows( ); i++ )
    {
        BOOST_CHECK_SMALL( combinedResiduals( i ) - expectedCombinedResiduals( i ), 1.0e-14 );
    }
}
...
{
    const std::vector< ObservableType > observableTypes = { position_observable, separation_distance };
    for( const ObservableType observableType : observableTypes )
    {
        ...
        simulation_setup::wrapObservationResiduals< double >( residuals, std::make_pair( 0, 3 ), observableType );
