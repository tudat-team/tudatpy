...
async function test() {
    ...
    const result = Module.runOrbitDetermination(JSON.stringify(initialGuess), JSON.stringify(truthState), 10800, 500, 100, 10, 'fullforce');

    const numIterations = Math.round(result[0]);
    const numObs = Math.round(result[1]);
    console.log('Converged in ' + numIterations + ' iterations, numObs=' + numObs);
