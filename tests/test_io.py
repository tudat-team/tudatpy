from tudatpy import data as io
import os
import pytest
import sys
...
def test_no_null_bytes():
    ...
    """
    assert "\x00" not in io.get_resource_path()
    assert "\x00" not in io.get_ephemeris_path()
    assert "\x00" not in io.get_earth_orientation_path()
    assert "\x00" not in io.get_quadrature_path()
    assert "\x00" not in io.get_spice_kernel_path()
    assert "\x00" not in io.get_atmosphere_tables_path()
    assert "\x00" not in io.get_gravity_models_path()
    assert "\x00" not in io.get_space_weather_path()


def test_paths_exist():
    """For linux which broke from osx's null byte fix."""
    assert os.path.exists(io.get_resource_path())
    assert os.path.exists(io.get_ephemeris_path())
    assert os.path.exists(io.get_earth_orientation_path())
    assert os.path.exists(io.get_quadrature_path())
    assert os.path.exists(io.get_spice_kernel_path())
    assert os.path.exists(io.get_atmosphere_tables_path())
    assert os.path.exists(io.get_gravity_models_path())
    assert os.path.exists(io.get_space_weather_path())
