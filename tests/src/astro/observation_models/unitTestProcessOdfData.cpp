...
BOOST_AUTO_TEST_CASE( testProcessOdfData )
{
    ...
    std::shared_ptr< input_output::OdfRawFileContents > rawOdfContents =
            std::make_shared< input_output::OdfRawFileContents >( odFile );
    ...
    std::shared_ptr< ProcessedOdfFileContents< Time > > processedOdfFileContents =
            std::make_shared< ProcessedOdfFileContents< Time > >(
                    rawOdfContents, spacecraftName, true );

    std::vector< observation_models::ObservableType > obsType =
            processedOdfFileContents->getProcessedObservableTypes( );
        ...
        for( const auto &linkEndsEntry: linkEndsMap )
        {
            ...
            for( const auto &observationSet: observationSetVector )
            {
                ...
                if( !observations.empty( ) && !observationTimes.empty( ) )
                {
                    ...
                    {
                        BOOST_CHECK_EQUAL( observation_models::getObservableName( observableType ),
                                           "NWayRange" );
                        ...
                        BOOST_CHECK_CLOSE_FRACTION(
                                double( observationTimes.at( 0 ) ), 234262616.184812933, 1e-9 );
                        BOOST_CHECK_CLOSE_FRACTION(
                                observations[ 0 ].transpose( )( 0 ), 333589.366953747, 1e-9 );

                        // Check ancillary settings
                        if( ancillarySettings != nullptr )
                        {
                            BOOST_CHECK_EQUAL( ancillarySettings->getDoubleData( ).at(
                                                       observation_models::
                                                               reception_reference_frequency_band ),
                                               1.000000000 );
                            BOOST_CHECK_CLOSE_FRACTION(
                                    ancillarySettings->getDoubleData( ).at(
                                            observation_models::
                            ...
                            BOOST_CHECK_CLOSE_FRACTION(
                                    ...
                                    14.000000000,
                                    1e-9 );
                            BOOST_CHECK_EQUAL( ancillarySettings->getDoubleVectorData( )
                                                       .at( observation_models::link_ends_delays )
                                                       .size( ),
                                               3 );
                            BOOST_CHECK_EQUAL( ancillarySettings->getDoubleVectorData( )
                                                       .at( observation_models::frequency_bands )
                                                       .size( ),
                                               2 );
                        }
                    }
                    ...
                    if( observableType == observation_models::dsn_n_way_averaged_doppler )
                    {
                        ...
                        BOOST_CHECK_EQUAL( linkEnds.at( receiver ).stationName_, "DSS-14" );
                        BOOST_CHECK_CLOSE_FRACTION(
                                double( observationTimes.at( 0 ) ), 234262457.184812993, 1e-9 );
                        BOOST_CHECK_CLOSE_FRACTION(
                                observations[ 0 ].transpose( )( 0 ), 1.563486099, 1e-9 );
