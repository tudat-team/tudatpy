...
BOOST_AUTO_TEST_CASE( testOneWayDoppplerModel )
{
    ...
    defaultBodySettings.get( "Sun" )->gravityFieldSettings =
            centralGravitySettings( spice_interface::getBodyGravitationalParameter( "Sun" ) * 10000.0 );
    ...
    bodies.at( "Spacecraft" )
            ->setEphemeris(
                    createBodyEphemeris( std::make_shared< KeplerEphemerisSettings >(
                                                 spacecraftOrbitalElements, 0.0, earthGravitationalParameter, "Earth", "ECLIPJ2000" ),
                                         "Spacecraft" ) );
    ...
    for( int useCorrections = 0; useCorrections < 3; useCorrections++ )
    {
        ...
        for( unsigned testCase = 0; testCase < 4; testCase++ )
        {
            ...
            if( useCorrections && testCase == 3 )
            {
                ...
                double lightTimeCorrectionUp = correction->calculateLightTimeCorrection( transmitterStateUp,
                                                                                         receiverStateUp,
                                                                                         observationTime + timePerturbation - lightTimeUp,
                                                                                         observationTime + timePerturbation );
                ...
                double lightTimeCorrectionDown = correction->calculateLightTimeCorrection( transmitterStateDown,
                                                                                           receiverStateDown,
                                                                                           observationTime - timePerturbation - lightTimeUp,
                                                                                           observationTime - timePerturbation );
...
BOOST_AUTO_TEST_CASE( testTwoWayDoppplerModel )
{
    ...
    // Create bodies settings needed in simulation
    BodyListSettings defaultBodySettings = getDefaultBodySettings( bodiesToCreate, "SSB" );
