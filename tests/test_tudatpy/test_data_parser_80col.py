...
def test_80cols_line_parser_logic():
    ...
    line_2025FA22 = (
        "     K25F22A  C2025 10 13.24277 00 20 45.76 +25 53 06.1          18.3 RrET147718"
    )
...
def test_parse_80cols_file_io(tmp_path):
    ...
    assert (
        len(parsed_table) == 1
    )  # must be one single observation line, since the second line is a satellite parallax line
...
@pytest.mark.parametrize(
    "row_data, expected_type, expected_name",
    [
