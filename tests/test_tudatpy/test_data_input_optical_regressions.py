...
def test_vfcc17_weights_are_partition_invariant_and_inputs_reusable(optical_table):
    table = pd.concat([optical_table] * 8, ignore_index=True)
    table["epoch"] += np.arange(8) / 86400.0
    whole, _ = read_optical_data(table, add_weights=True)
    first, _ = read_optical_data(table.iloc[:4], add_weights=True)
    second, _ = read_optical_data(table.iloc[4:], add_weights=True)
    bodies = weighting_bodies()
    expected = 0.5 / np.deg2rad(1.0 / 3600.0) ** 2
    # Exercise reuse after a smaller collection has already been created.
    small = create_observation_collection_from_tracking_data(first, bodies)
    np.testing.assert_allclose(small.concatenated_weights, 2.0 * expected, rtol=1.0e-13)
    for inputs in (whole, first + second, second + first):
        collection = create_observation_collection_from_tracking_data(inputs, bodies)
        assert len(collection.concatenated_weights) == 16
        np.testing.assert_allclose(collection.concatenated_weights, expected, rtol=1.0e-13)
    assert len(first[0].get_observation_weights()) == 0
    first[0].set_observation_weights([np.array([3.0, 4.0])] * 4)
    explicit = create_observation_collection_from_tracking_data(first, bodies)
...
def test_vfcc17_nightly_counts_do_not_mix_targets(optical_table):
    table = pd.concat([optical_table] * 8, ignore_index=True)
    table["number"] = ["433"] * 4 + ["1"] * 4
    data, _ = read_optical_data(table, add_weights=True)
    collection = create_observation_collection_from_tracking_data(data, weighting_bodies())
