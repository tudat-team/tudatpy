...
public:
     ...
     */
    RelativisticTimeStateDerivative( const std::pair< std::string, std::string >& referencePointId,
...
public:
     ...
     */
    DirectProperTimeRateStateDerivative( const std::shared_ptr< relativity::Metric > spaceTimeMetric,
        ...
        spaceTimeMetric_( spaceTimeMetric )
    {
        ...
        {
            throw std::runtime_error( "Error when making direct proper time state derivative, could not find body " +
        ...
        else
        {
            ...
            {
                referencePointStateFunction_ =
                        simulation_setup::getLinkEndCompleteCurrentStateFunctionDuringPropagation< double >( referencePointId, bodies );
            }
        }
    }
...
public:
        ...
        timeVariableConversionFunction_( timeVariableConversionFunction ), distanceScalingFactor_( distanceScalingFactor )
    {
        ...
        for( unsigned int i = 0; i < externalBodies.size( ); i++ )
        {
            ...
            {
                throw std::runtime_error( "Error, could not find body " + externalBodies.at( i ) +
            ...
            {
                throw std::runtime_error( "Error, could not find celestial body " + externalBodies.at( i ) +
                                          " in body map when making 1st order relativistic time converter" );
            }
            // Check if current body has a gravity field.
            else if( std::dynamic_pointer_cast< simulation_setup::Body >( bodies.getBody( externalBodies.at( i ) ) )
                             ->getGravityFieldModel( ) == nullptr )
            {
                throw std::runtime_error( "Error, celestial body " + externalBodies.at( i ) +
            ...
            else
            {
                ...
                this->externalBodyStateFunctions_.push_back( [ = ]( const TimeType& evaluationTime ) {
                    return body->getStateInBaseFrameFromEphemeris< double, TimeType >( evaluationTime );
                } );
                this->externalBodyGravitationalParameterFunctions_.push_back(
                        std::bind( &gravitation::GravityFieldModel::getGravitationalParameter, body->getGravityFieldModel( ) ) );
            }
        }
    }
...
template< typename StateScalarType = double, typename TimeType = double >
class FirstOrderBarycentricToBodyCentricTimeStateDerivative
    : public PostNewtonianRelativisticTimeStateDerivative< StateScalarType, TimeType >
{
public:
    ...
    FirstOrderBarycentricToBodyCentricTimeStateDerivative(
        ...
        PostNewtonianRelativisticTimeStateDerivative< StateScalarType, TimeType >( bodies,
        ...
        externalBodies_( externalBodies )
    {
        ...
        for( unsigned int i = 0; i < externalBodies.size( ); i++ )
        {
            ...
            {
                std::shared_ptr< simulation_setup::Body > currentCelestialBody =
                        std::dynamic_pointer_cast< simulation_setup::Body >( bodies.getBody( externalBodies.at( i ) ) );
                if( std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravityField >(
                            currentCelestialBody->getGravityFieldModel( ) ) == nullptr )
                {
                    throw std::runtime_error( "Error when making sh correction for relativistic time conversion, body " +
     ...
     *  \param currentGlobalTime Current time, in frame from which conversion is done (i.e. independent variable of equations of motion of
     * bodies)
     *  \param integratedValue Value of integral up to current time.
     *  \return Value of integrand at current time.
     */
    void calculateSystemStateDerivative( const TimeType currentGlobalTime,
                                         const Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 >& stateOfSystemToBeIntegrated,
                                         Eigen::Block< Eigen::Matrix< StateScalarType, Eigen::Dynamic, Eigen::Dynamic > > stateDerivative )
    {
        stateDerivative( 0, 0 ) =
     ...
     *  \param baseFrameTime Current time, in frame from which conversion is done (i.e. independent variable of equations of motion of
     * bodies)
     */
    void updateStateDerivativeModel( const TimeType baseFrameTime )
    {
        ...
        for( unsigned int i = 0; i < this->externalBodyStateFunctions_.size( ); i++ )
        {
            ...
            this->currentExternalBodyStates_[ i ].segment( 0, 3 ) =
                    this->currentExternalBodyStates_[ i ].segment( 0, 3 ) * this->distanceScalingFactor_;

            this->currentExternalBodyDistances_[ i ] =
                    ( this->currentCentralBodyState_.segment( 0, 3 ) - this->currentExternalBodyStates_[ i ].segment( 0, 3 ) ).norm( );
        }
    ...
    void updateBaseVariables( const TimeType& baseFrameTime )
    {
        ...
        for( unsigned int i = 0; i < this->externalBodyStateFunctions_.size( ); i++ )
        {
            this->currentExternalBodyGravitationalParameters_[ i ] =
                    this->externalBodyGravitationalParameterFunctions_[ i ]( ) * this->distanceScalingFactor_;
        }
    }
...
protected:
    ...
    std::map< int, std::function< double( Eigen::Vector3d ) > >
            higherOrderGravityFieldPotentialFunctions_;  // only dergee > 0 term contributions, added for 1st order variation.
...
template< typename StateScalarType = double, typename TimeType = double >
class SecondOrderBarycentricToBodyCentricTimeStateDerivative
    : public FirstOrderBarycentricToBodyCentricTimeStateDerivative< StateScalarType, TimeType >
{
public:
    //! Constructor.
    /*!
    ...
    SecondOrderBarycentricToBodyCentricTimeStateDerivative(
        ...
        FirstOrderBarycentricToBodyCentricTimeStateDerivative< StateScalarType, TimeType >(
                ...
                timeVariableConversionFunction,
                distanceScalingFactor,
                propagators::second_order_barycentric_to_bodycentric )
    {
        for( unsigned int i = 0; i < angularMomentumBodies.size( ); i++ )
        {
            ...
            //         std::dynamic_pointer_cast< simulation_setup::Body >( bodies.getBody( externalBodies.at( i ) ) );
            // this->externalBodyStateFunctions_.push_back( std::bind( &simulation_setup::Body::getStateInBaseFrameFromEphemeris,
            // bodies.getBody( externalBodies.at( i ) ), std::placeholders::_1 ) );
        }
                                         ...
                                         Eigen::Block< Eigen::Matrix< StateScalarType, Eigen::Dynamic, Eigen::Dynamic > > stateDerivative )
    {
        stateDerivative( 0, 0 ) =
                relativity::calculateFirstOrderTcbToTcgIntegrand( this->currentVelocity_, this->currentExternalPotential_ ) +
                relativity::calculateSecondOrderTcbToTcgIntegrand( this->currentVelocity_,
    ...
    void updateStateDerivativeModel( const TimeType baseFrameTime )
    {
        ...
        for( std::map< int, std::function< Eigen::Vector3d( ) > >::iterator momentaIterator = angularMomentumFunctions_.begin( );
             momentaIterator != angularMomentumFunctions_.end( );
             momentaIterator++ )
        {
        ...
        currentSecondOrderExternalPotentialCorrection_ =
                relativity::calculateSecondOrderExternalScalarPotentialCorrection( this->currentExternalBodyGravitationalParameters_,
                                                                                   this->currentExternalBodyDistances_,
                                                                                   currentBodySpeeds_,
                                                                                   this->currentExternalBodyStates_,
                                                                                   this->currentCentralBodyState_.segment( 3, 3 ) );
        currentExternalVectorPotential_ = relativity::calculateExternalVectorPotential( this->currentExternalBodyGravitationalParameters_,
                                                                                        this->currentExternalBodyDistances_,
                                                                                        this->currentExternalBodyStates_,
                                                                                        currentAngularMomenta_ );
    }
...
public:
    ...
    FirstOrderBodyCentricToTopoCentricTimeCalculator( const simulation_setup::SystemOfBodies& bodies,
        ...
        PostNewtonianRelativisticTimeStateDerivative< StateScalarType, TimeType >( bodies,
                                                                                   std::make_pair( centralBody, groundStation ),
                                                                                   externalBodies,
                                                                                   propagators::first_order_bodycentric_to_topocentric )
    {
        ...
        else
        {
            ...
            centralBodyRotationVector_ = std::bind( &ephemerides::RotationalEphemeris::getRotationalVelocityVectorInBaseFrame,
                                                    bodies.getBody( centralBody )->getRotationalEphemeris( ),
                                                    std::placeholders::_1 );
        ...
        {
            throw std::runtime_error( "Error, central body " + centralBody +
        ...
        else
        {
            ...
            {
                throw std::runtime_error( "Error, station " + groundStation + " not found on " + centralBody +
            ...
            else
            {
                ...
                std::shared_ptr< ground_stations::GroundStationState > groundStationState =
                        centralCelestialBody->getGroundStation( groundStation )->getNominalStationState( );
                if( !useTimeDependentBodyFixedPosition )
                {
                    pointPositionFunctionInPcrs_ =
                            std::bind( &ground_stations::GroundStationState::getNominalCartesianPosition, groundStationState );
                }
                else
                {
                    pointPositionFunctionInPcrs_ = [ = ]( double t ) {
                        return groundStationState->getCartesianPositionInTime( t, centralBody );
                    };
                }
                ...
                else
                {
                    ...
                    if( std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravityField >(
                                centralCelestialBody->getGravityFieldModel( ) ) == nullptr &&
                        maximumSphericalHarmonicDegree > 0 )
                    {
                        throw std::runtime_error(
                                "Error, requested spherical harmonic terms when making PCRS to TPRS time transformation, but no such model "
                                "found." );
                    }
                    ...
                    else
                    {
                        ...
                        auto sphericalHarmonicField = std::dynamic_pointer_cast< gravitation::SphericalHarmonicsGravityField >(
                                centralCelestialBody->getGravityFieldModel( ) );

                        localCentralBodyPotentialFunction_ = [ = ]( const Eigen::Vector3d& position ) {
                            return sphericalHarmonicField->getGravitationalPotential(
                                    position, maximumSphericalHarmonicDegree, maximumSphericalHarmonicDegree, 0, 0 );
                        };
                    }
                }
            }
        ...
        {
            throw std::runtime_error( "Error, acceleration term not yet implemented for PCRS to TPRS time transformation." );
        }
        else
        {
    ...
    {
        stateDerivative( 0, 0 ) =
                relativity::calculateFirstOrderPlanetocentricToTopocentricConversion( currentPointPositionInPcrs_,
    ...
    double getCurrentLocalPotentialAndTidalContribution( ) const
    {
        ...
        for( unsigned int i = 0; i < currentExternalBodyRelativePositions_.size( ); ++i )
        {
            ...
            total += this->currentExternalBodyGravitationalParameters_[ i ] / ( 2.0 * distance * distance * distance ) *
