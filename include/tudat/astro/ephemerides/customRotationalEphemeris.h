...
public:
     ...
     */
    CustomRotationalEphemeris( const std::function< Eigen::Quaterniond( const double ) > targetToBaseFrameOrientationFunction,
    ...
    virtual Eigen::Quaterniond getRotationToTargetFrame( const double secondsSinceEpoch )
    {
        return getRotationToBaseFrame( secondsSinceEpoch ).inverse( );
    }

    virtual Eigen::Matrix3d getDerivativeOfRotationToBaseFrame( const double secondsSinceEpoch )
    {
    ...
    virtual Eigen::Matrix3d getDerivativeOfRotationToTargetFrame( const double secondsSinceEpoch )
    {
