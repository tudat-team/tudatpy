...
public:
    ...
    marsDate( int y, int m, int d, double hh, double mm, double ss ):
    ...
    {
        return std::abs( basic_astrodynamics::convertCalendarDateToJulianDaysSinceEpoch( date1.year,
                                                                                         date1.month,
                                                                                         date1.day,
                                                                                         date1.hours,
                                                                                         date1.minutes,
                         ...
                         basic_astrodynamics::convertCalendarDateToJulianDaysSinceEpoch( date2.year,
                                                                                         date2.month,
                                                                                         date2.day,
                                                                                         date2.hours,
                                                                                         date2.minutes,
...
static std::vector< marsDate > marsYears = {
    marsDate( 1955, 4, 11, 0, 0, 0 ),  marsDate( 1957, 2, 26, 0, 0, 0 ),  marsDate( 1959, 1, 14, 0, 0, 0 ),
    marsDate( 1960, 12, 1, 0, 0, 0 ),  marsDate( 1962, 10, 19, 0, 0, 0 ), marsDate( 1964, 9, 5, 0, 0, 0 ),
    marsDate( 1966, 7, 24, 0, 0, 0 ),  marsDate( 1968, 6, 10, 0, 0, 0 ),  marsDate( 1970, 4, 28, 0, 0, 0 ),
    marsDate( 1972, 3, 15, 0, 0, 0 ),  marsDate( 1974, 1, 31, 0, 0, 0 ),  marsDate( 1975, 12, 19, 0, 0, 0 ),
    marsDate( 1977, 11, 5, 0, 0, 0 ),  marsDate( 1979, 9, 23, 0, 0, 0 ),  marsDate( 1981, 8, 10, 0, 0, 0 ),
    marsDate( 1983, 6, 28, 0, 0, 0 ),  marsDate( 1985, 5, 15, 0, 0, 0 ),  marsDate( 1987, 4, 1, 0, 0, 0 ),
    marsDate( 1989, 2, 16, 0, 0, 0 ),  marsDate( 1991, 1, 4, 0, 0, 0 ),   marsDate( 1992, 11, 21, 0, 0, 0 ),
    marsDate( 1994, 10, 9, 0, 0, 0 ),  marsDate( 1996, 8, 26, 0, 0, 0 ),  marsDate( 1998, 7, 14, 0, 0, 0 ),
    marsDate( 2000, 5, 31, 0, 0, 0 ),  marsDate( 2002, 4, 18, 0, 0, 0 ),  marsDate( 2004, 3, 5, 0, 0, 0 ),
    marsDate( 2006, 1, 21, 0, 0, 0 ),  marsDate( 2007, 12, 9, 0, 0, 0 ),  marsDate( 2009, 10, 26, 0, 0, 0 ),
    marsDate( 2011, 9, 13, 0, 0, 0 ),  marsDate( 2013, 7, 31, 0, 0, 0 ),  marsDate( 2015, 6, 18, 0, 0, 0 ),
    marsDate( 2017, 5, 5, 0, 0, 0 ),   marsDate( 2019, 3, 23, 0, 0, 0 ),  marsDate( 2021, 2, 7, 0, 0, 0 ),
    marsDate( 2022, 12, 26, 0, 0, 0 ), marsDate( 2024, 11, 12, 0, 0, 0 ), marsDate( 2026, 9, 30, 0, 0, 0 ),
...
{
public:
    MarsDtmAtmosphereModel( const std::function< double( const double ) > f107Function = []( const double ) { return 65.0; } );
    ...
    double computeLocalSolarTime( const double longitude,
                                  const int day,
                                  const int month,
                                  const int year,
                                  const int hours,
    ...
    double computeGl( const double latitude,
    ...
    double computeCurrentTemperature( const double latitude,
                                      const double longitude,
                                      const int minutes_,
                                      const int hours_,
                                      const int day_,
    ...
    double computeGamma( const double latitude,
                         const double longitude,
                         const int minutes_,
                         const int hours_,
                         const int day_,
    ...
    double heightDistributionFunction( const double altitude,
    ...
    double getTotalDensity( const double altitude,
                            const double latitude,
                            const double longitude,
                            const int minutes_,
                            const int hours_,
    ...
    double computeGl_Subr( const double latitude,
                           const double longitude,
                           const int minutes_,
                           const int hours_,
                           const int day_,
