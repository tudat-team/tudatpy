...
public:
        ...
        stationStates_( groundStationStates )
    {
        ...
        {
//            std::cerr<<
//                    "Warning when defining DSN N-way range observation model: the selected time type "
//                    "is not valid, using it would lead to large numerical errors."<<std::endl;
        ...
        if ( numberOfLinkEnds_ != 3 )
        {
            throw std::runtime_error(
                    "Error when defining DSN N-way range observation model: model allows exactly 3 link ends, " +
            ...
            const std::shared_ptr< ObservationAncilliarySimulationSettings > ancillarySettings = nullptr )
    {
        ...
        {
            throw std::runtime_error(
                "Error when computing DSN N-way range observables: the selected reference link end (" +
        ...
        {
            throw std::runtime_error(
                    "Error when simulating n-way DSN range observable; no ancillary settings found. " );
        }

        ObservationScalarType lowestRangingComponent; //referenceFrequency
...
//            referenceFrequency = ancillarySettings->getAncilliaryDoubleData( sequential_range_reference_frequency );
        ...
        {
            throw std::runtime_error(
                    "Error when retrieving frequency bands ancillary settings for DSN N-way range observable: "
