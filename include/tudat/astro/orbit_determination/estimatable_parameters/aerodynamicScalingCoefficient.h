...
public:
        ...
        timeLimits_( timeLimits )
    {
        ...
        for( unsigned int i = 0; i < aerodynamicAccelerations_.size( ); i++ )
        {
            ...
            {
                throw std::runtime_error( "Error when creating arc-wise estimated aerodynamic component scaling coefficient for " +
                                          associatedBody + ", current scaling coefficient not initialized" );
            }

            if( aerodynamicAccelerations_.at( i )->getComponentScalingFunction( parameterIndex_ ) != nullptr )
            {
                throw std::runtime_error( "Error when creating arc-wise estimated aerodynamic component scaling coefficient for " +
                                          associatedBody + ", time-varying scaling coefficient function is already defined" );
            }
        }
