...
void download_file_from_source( const char *remote_url, const char *filename )
{
    ...
    if( 0 != curl_ret )
    {
        ...
        throw std::runtime_error( "Error downloading file from: " + std::string( remote_url ) );
    }
    else if( !file_exists( filename ) )
    {
        throw std::runtime_error( "Downloaded file does not exist: " + std::string( filename ) );
    }
}
...
void unzip( const char *zip_file, const char *output_dir, int verbosity )
{
    ...
    {
        // throw unzip error
        throw std::runtime_error( "Error unzipping file: " + std::string( zip_file ) );
    }
    else if( !file_exists( output_dir ) )
    {
        throw std::runtime_error( "Unzipped directory does not exist: " + std::string( output_dir ) );
    }
}
