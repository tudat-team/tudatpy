...
public:
    ...
    std::pair< double, double > getTimeBoundsDouble( )
    {
        ...
        return std::make_pair( static_cast< double >( timeBounds.first ), static_cast< double >( timeBounds.second ) );
    }
    ...
    std::vector< std::pair< double, double > > getTimeBoundsPerSetDouble(
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        std::vector< std::pair< TimeType, TimeType > > timeBounds = getTimeBoundsPerSet( observationParser );
        ...
        for( auto obsIt : observationSetsIndices )
        {
            for( auto linkEndsIt : obsIt.second )
            {
                ...
                for( auto index : listRelevantSetIndices )
                {
                    ...
                    for( int k = singleSetStartAndSize.first; k < ( singleSetStartAndSize.first + singleSetStartAndSize.second ); k++ )
                    {
    ...
    std::vector< std::pair< double, double > > getTimeBoundsListDouble(
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        std::vector< std::pair< TimeType, TimeType > > timeBounds = getTimeBoundsList( observationParser );
        std::vector< std::pair< double, double > > doubleTimeBounds;
        doubleTimeBounds.reserve( timeBounds.size( ) );
    ...
    std::vector< std::vector< double > > getObservationTimesDouble(
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) ) const
    {
        ...
        for( const auto& times : observationTimes )
        {
            ...
            doubleObservationTimes.push_back( doubleTimesForSet );
        }

        return doubleObservationTimes;
    }
    ...
    std::pair< std::vector< Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 > >, std::vector< std::vector< double > > >
    getObservationsAndTimesDouble(
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) )
    {
        return std::make_pair( getObservations( observationParser ), getObservationTimesDouble( observationParser ) );
    }
    ...
    // Double version of getConcatenatedObservationsAndTimes
    std::pair< Eigen::Matrix< ObservationScalarType, Eigen::Dynamic, 1 >, std::vector< double > > getConcatenatedObservationsAndTimesDouble(
            std::shared_ptr< ObservationCollectionParser > observationParser = std::make_shared< ObservationCollectionParser >( ) )
    {
                ...
                for( auto link_end_it : obs_it.second )
                {
                    ...
                    else
                    {
                        ...
                        observationSetList_[ obs_it.first ][ link_end_it.first ].insert(
                                observationSetList_[ obs_it.first ][ link_end_it.first ].end( ), listToAdd.begin( ), listToAdd.end( ) );
                    }
                }
            }
        }
    ...
    // Double version of getDependentVariableHistoryPerObservationSet
    std::vector< std::map< double, Eigen::VectorXd > > getDependentVariableHistoryPerObservationSetDouble(
            std::shared_ptr< simulation_setup::ObservationDependentVariableSettings > dependentVariableSettings,
            const bool returnFirstCompatibleSettings = false,
            const std::shared_ptr< ObservationCollectionParser > parser = std::make_shared< ObservationCollectionParser >( ) )
    {
        // Get TimeType version
        std::vector< std::map< TimeType, Eigen::VectorXd > > timeTypeHistories =
                getDependentVariableHistoryPerObservationSet( dependentVariableSettings, returnFirstCompatibleSettings, parser );

        // Convert each map to double keys
    ...
    // Double version of getDependentVariableHistory
    std::map< double, Eigen::VectorXd > getDependentVariableHistoryDouble(
            std::shared_ptr< simulation_setup::ObservationDependentVariableSettings > dependentVariableSettings,
            const bool returnFirstCompatibleSettings = false,
            const std::shared_ptr< ObservationCollectionParser > parser = std::make_shared< ObservationCollectionParser >( ) )
    {
        // Get TimeType version and convert to double using staticCastMapKeys
        return utilities::staticCastMapKeys< double, TimeType, Eigen::VectorXd >(
                getDependentVariableHistory( dependentVariableSettings, returnFirstCompatibleSettings, parser ) );
    }
...
template< typename ObservationScalarType = double, typename TimeType = double >
std::shared_ptr< ObservationCollection< ObservationScalarType, TimeType > > mergeObservationCollections(
        ...
        const std::shared_ptr< TrackingData< ObservationScalarType, TimeType > > trackingData )
{
    ...
    // Retrieve ancillary settings maps from TrackingData object
    std::map< std::string, std::vector< std::string > > ancillarySettingsStringVector = trackingData->getAncillarySettingsStringVector( );

    for( auto it : ancillarySettingsStringVector )
    {
    }

    std::map< std::string, double > ancillarySettingsDouble = trackingData->getAncillarySettingsDouble( );

    std::map< std::string, vector< double > > ancillarySettingsDoubleVector = trackingData->getAncillarySettingsDoubleVector( );
        ...
        const std::shared_ptr< TrackingData< ObservationScalarType, TimeType > > trackingData )
{
    ...
    {
        ancillarySettings->setAncillaryDoubleData( getAncillaryVariableFromString( it.first ), it.second );
    }

    // Parse and add ancillary settings of type double vectors
    for( auto& it : trackingData->getAncillarySettingsDoubleVector( ) )
    {
        ancillarySettings->setAncillaryDoubleVectorData( getAncillaryVariableFromString( it.first ), it.second );
    }
    ...
    {
        ObservationAncillarySimulationVariable ancillaryVariable = getAncillaryVariableFromString( it.first );
        std::vector< std::string > ancillaryValues = it.second;

        // Check consistency between ancillary variable and value type
        if( ancillaryVariable != ( reception_reference_frequency_band || frequency_bands ) )
        {
            throw runtime_error( "Error when setting ancillary simulation settings from tracking data, inconsistency between 
                the ancillary variable (" + it.first + ") and the type of ancillary value provided (string)." );
        }
        ...
        {
            bandDoubles.push_back( convertFrequencyBandToDouble( getFrequencyBandFromString( band ) ) );
        }

        if( ancillaryVariable == reception_reference_frequency_band )
        {
            if( bandDoubles.size( ) != 1 )
            {
                throw std::runtime_error( "Error when setting reception_reference_frequency_band ancillay settings, 
                    expects exactly one band." );
            }

            // Set reference frequency band as double
            ancillarySettings->setAncillaryDoubleData( variable, bandDoubles.front( ) );
        }
        else if( ancillaryVariable == frequency_bands )
        {
            // Set frequency bands as double vector
            ancillarySettings->setAncillaryDoubleVectorData( variable, bandDoubles );
        }
    }
        ...
        const bool applyCorrections = false )
{
    ...
    observation_models::LinkEndType referenceLinkEnd = getLinkEndTypeFromString( trackingData->getReferenceLinkEnd( ) );
    ...
    checkTrackingDataLinkEnds( observableType, linkEnds, referenceLinkEnd );
    ...
    std::shared_ptr< SingleObservationSet< ObservationScalarType, TimeType > > observationSet =
            std::make_shared< SingleObservationSet< ObservationScalarType, TimeType > >(
                    observableType, linkEnds, observations, trackingData->getObservationEpochs( ), referenceLinkEnd, ancillarySettings );
