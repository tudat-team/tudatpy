...
#ifndef TUDAT_CREATEESTIMATABLEPARAMETERSFACTORY_H
#define TUDAT_CREATEESTIMATABLEPARAMETERSFACTORY_H

#include "tudat/simulation/estimation_setup/createEstimatableParametersInterface.h"
    ...
    for( auto it : bodyMappingIndices )
    {
        ...
        for( unsigned int i = 0; i < arcAndBodyIndices.size( ); i++ )
        {
            ...
            initialStates.segment( i * 6, 6 ) = singleArcTranslationalSettings.at( currentArcAndBodyIndex.first )
                                                        ->getInitialStates( )
                                                        .segment( 6 * currentArcAndBodyIndex.second, 6 );
    ...
    else
    {
        ...
        switch( vectorParameterName->parameterType_.first )
        {
            ...
            case arcwise_constant_additive_observation_bias: {
                ...
                {
                    vectorParameterToEstimate = std::make_shared< MultiArcObservationBiasParameter >(
...
#endif  // TUDAT_CREATEESTIMATABLEPARAMETERSFACTORY_H
