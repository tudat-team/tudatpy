 ...
 */
double determineChebyshevPolynomialIndependentVariable( const double intervalStart,
 ...
 */
std::vector< double > evaluateChebyshevPolynomials( const double intervalStart,
                                                    const double intervalEnd,
                                                    const double evaluationPoint,
 ...
 *  \return Vector of basis values \f$T_0,\ldots,T_N\f$.
 */
std::vector< double > evaluateChebyshevPolynomials( const double independentChebyshevVariable, const int maximumOrder );

//! Chebyshev interpolator class.
template< typename IndependentVariableType, typename DependentVariableType >
class ChebyshevInterpolator : public OneDimensionalInterpolator< IndependentVariableType, DependentVariableType >
{
public:
     ...
     */
    ChebyshevInterpolator( const std::map< IndependentVariableType, std::vector< DependentVariableType > >& chebyshevCoefficients,
                           const AvailableLookupScheme selectedLookupScheme = huntingAlgorithm )
    {
        if( chebyshevCoefficients.size( ) < 2 )
        {
            throw std::runtime_error( "Error: ChebyshevInterpolator requires at least 2 independent values." );
        }
        ...
        for( const auto& entry : chebyshevCoefficients )
        {
            ...
            {
                throw std::runtime_error( "Error: Inconsistent Chebyshev coefficient vector sizes across segments." );
            }
     ...
     *  \param selectedLookupScheme Lookup scheme used to locate current interpolation segment.
     */
    ChebyshevInterpolator( const std::vector< IndependentVariableType >& independentValues,
                           ...
                           const AvailableLookupScheme selectedLookupScheme = huntingAlgorithm )
    {
        ...
        {
            throw std::runtime_error( "Error: Insufficient Chebyshev coefficient sets for provided independent values." );
        }
    ...
    DependentVariableType interpolate( const IndependentVariableType independentVariableValue )
    {
        ...
        else if( independentVariableValue >=
                 independentValues_.back( ) + 0.1 * ( independentValues_.back( ) - independentValues_[ independentValues_.size( ) - 2 ] ) )
        {
        ...
        else
        {
            ...
            if( nearestLowerIndex < 0 || static_cast< unsigned int >( nearestLowerIndex + 1 ) >= independentValues_.size( ) ||
                static_cast< unsigned int >( nearestLowerIndex ) >= chebyshevCoefficients_.size( ) )
            {
                throw std::runtime_error( "Error: Invalid segment index encountered in ChebyshevInterpolator::interpolate." );
            }
            const auto& coeffs = chebyshevCoefficients_[ nearestLowerIndex ];
            auto chebyshevPolynomials = evaluateChebyshevPolynomials( independentValues_[ nearestLowerIndex ],
