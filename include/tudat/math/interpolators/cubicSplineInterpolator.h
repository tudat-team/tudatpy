...
public:
    ...
    DependentVariableType interpolate( const IndependentVariableType targetIndependentVariableValue )
    {
        ...
        const ScalarType interval = getTimeDifference< ScalarType >( upperValue, lowerValue );
        squareDifference = interval * interval;
        ScalarType coefficientA_ = getTimeDifference< ScalarType >( upperValue, targetIndependentVariableValue ) / interval;
...
private:
    ...
    void calculateSecondDerivatives( )
    {
        ...
        {
            hCoefficients_[ i ] = getTimeDifference< ScalarType >( independentValues_[ i + 1 ], independentValues_[ i ] );
        }
