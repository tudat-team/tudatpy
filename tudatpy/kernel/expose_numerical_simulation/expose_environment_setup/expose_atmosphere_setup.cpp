 ...
 */
 ...
 .. code-block:: python
    ...
    body_settings.get( "Earth" ).atmosphere_settings = environment_setup.atmosphere.exponential(
     ...
     m.def( "nrlmsise00",
            ...
            py::arg( "geomagnetic_activity" ) = -1,
            R"doc(
 
 Function for creating NRLMSISE-00 atmospheric model settings.
 
 Function for settings object, defining atmosphere model in accordance to the NRLMSISE-00 global reference model for Earth's atmosphere with optional geomagnetic activity switch.
 ...
 geomagnetic_activity : int, optional
     Controls the geomagnetic activity behavior. Default (-1) uses full vector of AP values. Set to 1 for quite magnetic activity, using the daily ap value.
     Returns
 -------
 AtmosphereSettings
