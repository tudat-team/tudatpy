...
Same as ``use_boundary_value``, but a warning is printed to the terminal
         ...
         return R"(
The interpolation scheme is extended beyond the range `t_{0}...t_{N}` without any warning. That is, the mathematical equation used to compute the value of :math:`x` in the range :math:`[t_{0}..t_{1}]` is used without any checks for :math:`t<t_{0}`  (and equivalently for :math:`t>t_{N}`). Warning, using this setting can result in divergent/unrealistic behaviour
     )";
...
Parameters
----------
lookup_scheme : AvailableLookupScheme, default=huntingAlgorithm
...
lookup_scheme : AvailableLookupScheme, default = huntingalgorithm
    Algorithm used to find the nearest neighbor in the independent variable input data when the interpolation scheme is called
boundary_interpolation : BoundaryInterpolationType = extrapolate_at_boundary_with_warning
    Interpolator behaviour that is to be used beyond the upper and lower boundaries of the independent variable input data
Returns
-------
InterpolatorSettings
...
boundary_interpolation : BoundaryInterpolationType = extrapolate_at_boundary_with_warning
    Interpolator behaviour that is to be used beyond the upper and lower boundaries of the independent variable input data
lagrange_boundary_handling : LagrangeInterpolatorBoundaryHandling = lagrange_cubic_spline_boundary_interpolation
...
absolute_error_tolerance : float
    Value of relative error tolerance :math:`\epsilon_{a}`.
...
absolute_error_tolerance : numpy.ndarray[numpy.float64[m, n]]
    Values of relative error tolerance :math:`\boldsymbol{\epsilon_{a}`.
safety_factor : float. default = 0.8
    Safety factor :math:`K` for step size control
minimum_factor_increase : float, default = 0.1
    Minimum permissible value for :math:`\Delta t_{rec.}/\Delta t`
