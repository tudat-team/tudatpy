...
class SimulationVariableDict(dict):
    """
    Dictionary-like class designed to store and retrieve dependent variable histories
    using `SingleDependentVariableSaveSettings` objects.
    """
    ...
    def __setitem__(self, __key: SingleDependentVariableSaveSettings, __value: any) -> None:
        return super().__setitem__(self.__read_key(__key), __value)

    def __getitem__(self, __key: SingleDependentVariableSaveSettings) -> any:
        return super().__getitem__(self.__read_key(__key))
    
    def __read_key(self, key):

        # key: dependent variable ID obtained from SingleDependentVariableSaveSettings object
        if isinstance(key, SingleDependentVariableSaveSettings):
            key = get_dependent_variable_id(key)
        
        # ensure key is a string by this point
        if not isinstance(key, str): raise TypeError(
            'SimulationVariableDict keys must be either `SingleDependentVariableSaveSettings` or dependent variable ID strings.'
        )
...
def result2dict(
    ...
    """
    Create a dictionary of dependent variables, containing an `n` by `m` dependent variable matrix
    for each dependent variable saved, where `n` the number of dimensions of the dependent variable 
    (1 for a scalar, >1 for vectorial variables) and `m` is the number of propagated epochs.

    SimulationVariableDict[str: dict(float, float/np.ndarray[])]
    ...
    # retrieve dependent variable IDs
    dependent_variable_id_dictionary = dynamics_simulator.propagation_results.dependent_variable_ids

    # retrieve saved variable dimensions
    dependent_variable_dimensions = [dimension for index, dimension in dependent_variable_id_dictionary.keys()]

    # dependent variable string IDs
    dependent_variable_ids = [description for description in dependent_variable_id_dictionary.values()]

    # retrieve /transposed/ dependent variable history array
    time_history_array = result2array(dynamics_simulator.dependent_variable_history).T[0, :]
    dependent_variable_history_array = result2array(dynamics_simulator.dependent_variable_history).T[1:, :]

    # construct dependent variable matrices
    dependent_variable_matrices = []
    for dimension in dependent_variable_dimensions:
        
        # save dependent variable matrix, /transposing it back/ to the desired m x n
        dependent_variable_matrices.append(
            dependent_variable_history_array[:dimension, :]
        )
        dependent_variable_history_array = np.delete(dependent_variable_history_array, np.s_[:dimension], axis=0)

    # construct dependent variable dictionary
    dependent_variable_dictionary = SimulationVariableDict({
        dependent_variable: {
            epoch: dependent_variable_matrices[i][:, j]
        }
        for i, dependent_variable in enumerate(dependent_variable_ids)
        for j, epoch in enumerate(time_history_array)
    })
